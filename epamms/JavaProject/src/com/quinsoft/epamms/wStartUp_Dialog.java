...
ProcessUserLogin( View     ViewToWindow )
{
   ...
   if ( szTempString_1 == null )
      ...
      sb_szTempString_1 = new StringBuilder( szTempString_1 );
       GetStringFromAttribute( sb_szTempString_1, wWebXfer, "Root", "LoginName" );
...
private int 
o_fnLocalBuildQual_42( View     vSubtask,
                       zVIEW    vQualObject )
{
   int      RESULT = 0;
...
private int 
o_fnLocalBuildQual_43( View     vSubtask,
                       zVIEW    vQualObject )
{
   int      RESULT = 0;
...
//:DIALOG OPERATION
//:InitFontForAdd( VIEW ViewToWindow )
...
InitFontForAdd( View     ViewToWindow )
{
   ...
   //:CREATE ENTITY mSubreg.Font
   RESULT = CreateEntity( mSubreg, "Font", zPOS_AFTER );
   //:mSubreg.Font.wkCreated = "Y"
   SetAttributeFromString( mSubreg, "Font", "wkCreated", "Y" );
   return( 0 );
// END
} 
...
//:DIALOG OPERATION
//:InitFontForUpdate( VIEW ViewToWindow )
...
InitFontForUpdate( View     ViewToWindow )
{
   ...
   //:mSubreg.Font.wkCreated = "N"
   SetAttributeFromString( mSubreg, "Font", "wkCreated", "N" );
   return( 0 );
// END
} 
...
//:DIALOG OPERATION
//:SaveAndAddNewFont( VIEW ViewToWindow )
...
CancelFont( View     ViewToWindow )
{
   ...
   //:IF mSubreg.Font.wkCreated = "Y"
   if ( CompareAttributeToString( mSubreg, "Font", "wkCreated", "Y" ) == 0 )
   { 
      //:DELETE ENTITY mSubreg.Font
      RESULT = DeleteEntity( mSubreg, "Font", zPOS_NEXT );
   } 
...
//:DIALOG OPERATION
//:ResumeEditingSPLD( VIEW ViewToWindow )
...
ResumeEditingSPLD( View     ViewToWindow )
{
   ...
   //:STRING ( 64 ) szKeyAttributeName
   String   szKeyAttributeName = null;
   //:INTEGER lKeyAttributeValue
   int      lKeyAttributeValue = 0;
   ...
   { 
      ...
      { 
         //:szLOD = mUser.ResumeObject.LOD
         {MutableInt mi_lTempInteger_1 = new MutableInt( lTempInteger_1 );
         StringBuilder sb_szLOD;
         if ( szLOD == null )
            sb_szLOD = new StringBuilder( 32 );
         else
            sb_szLOD = new StringBuilder( szLOD );
                   GetVariableFromAttribute( sb_szLOD, mi_lTempInteger_1, 'S', 65, mUser, "ResumeObject", "LOD", "", 0 );
         lTempInteger_1 = mi_lTempInteger_1.intValue( );
         szLOD = sb_szLOD.toString( );}
         //:szEntityName = mUser.ResumeObject.EntityName
         {MutableInt mi_lTempInteger_2 = new MutableInt( lTempInteger_2 );
         ...
         if ( szEntityName == null )
            ...
            sb_szEntityName = new StringBuilder( szEntityName );
                   GetVariableFromAttribute( sb_szEntityName, mi_lTempInteger_2, 'S', 65, mUser, "ResumeObject", "EntityName", "", 0 );
         lTempInteger_2 = mi_lTempInteger_2.intValue( );
         szEntityName = sb_szEntityName.toString( );}
         //:szKeyAttributeName = mUser.ResumeObject.KeyAttributeName
         {MutableInt mi_lTempInteger_3 = new MutableInt( lTempInteger_3 );
         StringBuilder sb_szKeyAttributeName;
         if ( szKeyAttributeName == null )
            sb_szKeyAttributeName = new StringBuilder( 32 );
         else
            sb_szKeyAttributeName = new StringBuilder( szKeyAttributeName );
                   GetVariableFromAttribute( sb_szKeyAttributeName, mi_lTempInteger_3, 'S', 65, mUser, "ResumeObject", "KeyAttributeName", "", 0 );
         lTempInteger_3 = mi_lTempInteger_3.intValue( );
         szKeyAttributeName = sb_szKeyAttributeName.toString( );}
         //:lKeyAttributeValue = mUser.ResumeObject.KeyAttributeValue
         {MutableInt mi_lKeyAttributeValue = new MutableInt( lKeyAttributeValue );
                   GetIntegerFromAttribute( mi_lKeyAttributeValue, mUser, "ResumeObject", "KeyAttributeValue" );
         lKeyAttributeValue = mi_lKeyAttributeValue.intValue( );}
         //:szViewName = mUser.ResumeObject.ViewName
         {MutableInt mi_lTempInteger_4 = new MutableInt( lTempInteger_4 );
         ...
         if ( szViewName == null )
            ...
            sb_szViewName = new StringBuilder( szViewName );
                   GetVariableFromAttribute( sb_szViewName, mi_lTempInteger_4, 'S', 65, mUser, "ResumeObject", "ViewName", "", 0 );
         lTempInteger_4 = mi_lTempInteger_4.intValue( );
         szViewName = sb_szViewName.toString( );}
         //:GET VIEW vTemp NAMED szViewName
         RESULT = GetViewByName( vTemp, szViewName, ViewToWindow, zLEVEL_TASK );
         //:IF vTemp != 0
         if ( getView( vTemp ) != null )
         { 
            //:DropView( vTemp )
            DropView( vTemp );
         } 
         ...
         { 
            ...
            //:BuildCompEntriesForSPLD( vTemp )
            {
             ZGlobalS_Operation m_ZGlobalS_Operation = new ZGlobalS_Operation( vTemp );
             m_ZGlobalS_Operation.BuildCompEntriesForSPLD( vTemp );
   ...
   { 
      //:MessageSend( ViewToWindow, "", "Resume",
      //:             "Resume to SPLD not available",
      //:             zMSGQ_OBJECT_CONSTRAINT_ERROR, 0 )
      MessageSend( ViewToWindow, "", "Resume", "Resume to SPLD not available", zMSGQ_OBJECT_CONSTRAINT_ERROR, 0 );
      //:RETURN 2
      if(8==8)return( 2 );
   } 
