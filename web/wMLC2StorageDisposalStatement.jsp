...
if ( strLastWindow.equals("wMLC2StorageDisposalStatement") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wMLC2StorageDisposalStatement strActionToProcess *** " + strActionToProcess );
task.log().info("*** wMLC2StorageDisposalStatement LastWindow *** " + strLastWindow );
task.log().info("*** wMLC2StorageDisposalStatement LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wMLC2StorageDisposalStatement", strActionToProcess );
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wMLC2StorageDisposalStatement.jsp", "wMLC2.AcceptAddNewStorDispStmt" );
      try
      {
         nOptRC = wMLC2.AcceptAddNewStorDispStmt( new zVIEW( vKZXMLPGO ) );
      }
      catch (Exception e)
      {
         // Set the error return code.
         nOptRC = 2;
         strVMLError = "<br><br>*** Error running Operation AcceptAddNewStorDispStmt: " + e.getMessage();
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "CleanStorDispStmtHTML" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wMLC2StorageDisposalStatement.jsp", "wMLC2.CleanStorDispStmtHTML" );
      try
      {
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----------------------------------->>> " + "wMLC2StorageDisposalStatement" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wMLC2StorageDisposalStatement.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wMLC2StorageDisposalStatement.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<script language="JavaScript" type="text/javascript" src="./genjs/wMLC2StorageDisposalStatement.js"></script>
...
<form name="wMLC2StorageDisposalStatement" id="wMLC2StorageDisposalStatement" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wMLC2", "StorageDisposalStatement" );
...
<%
   session.setAttribute( "ZeidonWindow", "wMLC2StorageDisposalStatement" );
