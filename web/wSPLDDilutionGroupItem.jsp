...
<%-- wSPLDDilutionGroupItem   Generate Timestamp: 20170510135611861 --%>
                           ...
                           boolean webMapping )
{
   ...
   {
      // EditBox: EditBox3
      nRC = mSPLDef.cursor( "SPLD_DilutionGroupItem" ).checkExistenceOfEntity( ).toInt();
      if ( nRC >= 0 ) // CursorResult.SET
      {
         ...
         try
         {
            if ( webMapping )
               ...
               mSPLDef.cursor( "SPLD_DilutionGroupItem" ).getAttribute( "Text" ).setValue( strMapValue, "" );
         }
...
if ( strLastWindow.equals("wSPLDDilutionGroupItem") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSPLDDilutionGroupItem strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSPLDDilutionGroupItem LastWindow *** " + strLastWindow );
task.log().info("*** wSPLDDilutionGroupItem LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSPLDDilutionGroupItem", strActionToProcess );
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wSPLDDilutionGroupItem" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSPLDDilutionGroupItem.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSPLDDilutionGroupItem.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<title>SPLD Dilution Group Item</title>
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSPLDDilutionGroupItem.js"></script>
...
<%
   ...
   {
%>
       <li id="Return" name="Return"><a href="#"  onclick="CancelGroupItem()">Return</a></li>
<%
   }
%>
...
<form name="wSPLDDilutionGroupItem" id="wSPLDDilutionGroupItem" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSPLD", "DilutionGroupItem" );
...
<div style="height:1px;width:12px;float:left;"></div>   <!-- Width Spacer -->
<% /* GroupBox7:GroupBox */ %>
<div id="GroupBox7" name="GroupBox7" style="float:left;width:544px;" >
   ...
   else
   {
      ...
      else
      {
         ...
         if ( nRC >= 0 )
         {
            try
            {
               strErrorMapValue = mSPLDef.cursor( "SPLD_DilutionGroupItem" ).getAttribute( "Text" ).getString( "" );
            }
            catch (Exception e)
            {
            ...
            task.log( ).debug( "SPLD_DilutionGroupItem.Text: " + strErrorMapValue );
         }
         else
            task.log( ).debug( "Entity does not exist for EditBox3: " + "mSPLDef.SPLD_DilutionGroupItem" );
      }
   }
%>

<input class="text12"  name="EditBox3" id="EditBox3" maxlength="4096" style="width:426px;<%=strErrorColor%>" type="text" value="<%=strErrorMapValue%>" >
...
<%
   session.setAttribute( "ZeidonWindow", "wSPLDDilutionGroupItem" );
