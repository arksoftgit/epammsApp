...
<%-- wSLCTableSpecifications   Generate Timestamp: 20161014104443893 --%>
      ...
      if ( nRC >= 0 ) // CursorResult.SET
      {
         ...
         try
         {
            if ( webMapping )
               ...
               mSubLC.cursor( "S_MetaTable" ).getAttribute( "Name" ).setValue( strMapValue, "" );
         }
         catch ( InvalidAttributeValueException e )
         {
            nMapError = -16;
      ...
      if ( nRC >= 0 ) // CursorResult.SET
      {
         ...
         try
         {
            if ( webMapping )
               ...
               mSubLC.cursor( "S_MetaTable" ).getAttribute( "Description" ).setValue( strMapValue, "" );
         }
         catch ( InvalidAttributeValueException e )
         {
            nMapError = -16;
...
if ( strLastWindow.equals("wSLCTableSpecifications") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSLCTableSpecifications strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSLCTableSpecifications LastWindow *** " + strLastWindow );
task.log().info("*** wSLCTableSpecifications LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSLCTableSpecifications", strActionToProcess );
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wSLCTableSpecifications" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSLCTableSpecifications.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSLCTableSpecifications.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSLCTableSpecifications.js"></script>
...
<form name="wSLCTableSpecifications" id="wSLCTableSpecifications" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSLC", "TableSpecifications" );
   ...
   else
   {
      ...
      else
      {
         ...
         if ( nRC >= 0 )
         {
            try
            {
               strErrorMapValue = mSubLC.cursor( "S_MetaTable" ).getAttribute( "Name" ).getString( "" );
            }
            catch (Exception e)
            {
            ...
            task.log( ).debug( "S_MetaTable.Name: " + strErrorMapValue );
         }
         else
            task.log( ).debug( "Entity does not exist for TableName: " + "mSubLC.S_MetaTable" );
      }
   }
%>
   ...
   else
   {
      ...
      else
      {
         ...
         if ( nRC >= 0 )
         {
            try
            {
               strErrorMapValue = mSubLC.cursor( "S_MetaTable" ).getAttribute( "Description" ).getString( "" );
            }
            catch (Exception e)
            {
            ...
            task.log( ).debug( "S_MetaTable.Description: " + strErrorMapValue );
         }
         else
            task.log( ).debug( "Entity does not exist for Description: " + "mSubLC.S_MetaTable" );
      }
   }
%>
...
<% /* GridTable:Grid */ %>
<table  cols=1 style="position:absolute;top:112px;left:8px;"  name="GridTable" id="GridTable">
...
<%
   session.setAttribute( "ZeidonWindow", "wSLCTableSpecifications" );
