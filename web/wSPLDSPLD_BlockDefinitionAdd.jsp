...
<%-- wSPLDSPLD_BlockDefinitionAdd   Generate Timestamp: 20160531205246365 --%>
...
if ( strLastWindow.equals("wSPLDSPLD_BlockDefinitionAdd") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSPLDSPLD_BlockDefinitionAdd strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSPLDSPLD_BlockDefinitionAdd LastWindow *** " + strLastWindow );
task.log().info("*** wSPLDSPLD_BlockDefinitionAdd LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSPLDSPLD_BlockDefinitionAdd", strActionToProcess );

      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLDSPLD_BlockDefinitionAdd", "wSPLD.CANCEL_BlockSubBlockDefinition" );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "CONTINUE_BlockSubBlockDefAdd" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLDSPLD_BlockDefinitionAdd", "wSPLD.CONTINUE_BlockSubBlockDefAdd" );
      nOptRC = wSPLD.CONTINUE_BlockSubBlockDefAdd( new zVIEW( vKZXMLPGO ) );
      if ( nOptRC == 2 )
      {
         nRC = 2;  // do the "error" redirection
         session.setAttribute( "ZeidonError", "Y" );
      ...
      {
         // Next Window
         strNextJSP_Name = wSPLD.SetWebRedirection( vKZXMLPGO, wSPLD.zWAB_ReplaceWindowWithModalWindow, "wSPLD", "SPLD_BlockDefinitionUpdate" );
      }
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wSPLDSPLD_BlockDefinitionAdd" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSPLDSPLD_BlockDefinitionAdd.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSPLDSPLD_BlockDefinitionAdd.jsp") ) 
         {
            response.sendRedirect( strURL );
...
else
{
   ...
   if ( VmlOperation.isValid( wWebXA ) )
   {
      ...
      wWebXA.cursor( "Root" ).getAttribute( "CurrentWindow" ).setValue( "SPLD_BlockDefinitionAdd", "" );
   }
...
<title>Add Block Definition to SPLD</title>
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSPLDSPLD_BlockDefinitionAdd.js"></script>
...
<%
   ...
   {
%>
       <li id="Continue" name="Continue"><a href="#"  onclick="CONTINUE_BlockSubBlockDefAdd()">Continue</a></li>
<%
   }
%>
...
<form name="wSPLDSPLD_BlockDefinitionAdd" id="wSPLDSPLD_BlockDefinitionAdd" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSPLD", "SPLD_BlockDefinitionAdd" );
...
<% strTextDisplayValue = "";
   ...
   else
   {
      ...
      try
      {
         strTextDisplayValue = mSPLDef.cursor( "LLD_Block" ).getAttribute( "Name" ).getString( "" );
      }
      catch (Exception e)
      {
