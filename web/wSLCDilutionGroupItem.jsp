...
<%-- wSLCDilutionGroupItem   Generate Timestamp: 20170510134559288 --%>
                           ...
                           boolean webMapping )
{
   ...
   {
      // EditBox: EditBox3
      nRC = mSubLC.cursor( "S_DilutionGroupItem" ).checkExistenceOfEntity( ).toInt();
      if ( nRC >= 0 ) // CursorResult.SET
      {
         ...
         try
         {
            if ( webMapping )
               ...
               mSubLC.cursor( "S_DilutionGroupItem" ).getAttribute( "Text" ).setValue( strMapValue, "" );
         }
...
if ( strLastWindow.equals("wSLCDilutionGroupItem") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSLCDilutionGroupItem strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSLCDilutionGroupItem LastWindow *** " + strLastWindow );
task.log().info("*** wSLCDilutionGroupItem LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSLCDilutionGroupItem", strActionToProcess );
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wSLCDilutionGroupItem" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSLCDilutionGroupItem.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSLCDilutionGroupItem.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<title>SLC Dilution Group Item</title>
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSLCDilutionGroupItem.js"></script>
...
<form name="wSLCDilutionGroupItem" id="wSLCDilutionGroupItem" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSLC", "DilutionGroupItem" );
   ...
   else
   {
      ...
      else
      {
         ...
         if ( nRC >= 0 )
         {
            try
            {
               strErrorMapValue = mSubLC.cursor( "S_DilutionGroupItem" ).getAttribute( "Text" ).getString( "" );
            }
            catch (Exception e)
            {
            ...
            task.log( ).debug( "S_DilutionGroupItem.Text: " + strErrorMapValue );
         }
         else
            task.log( ).debug( "Entity does not exist for EditBox3: " + "mSubLC.S_DilutionGroupItem" );
      }
   }
%>
...
<%
   session.setAttribute( "ZeidonWindow", "wSLCDilutionGroupItem" );
