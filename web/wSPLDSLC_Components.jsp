...
if ( strLastWindow.equals("wSPLDSLC_Components") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSPLDSLC_Components strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSPLDSLC_Components LastWindow *** " + strLastWindow );
task.log().info("*** wSPLDSLC_Components LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSPLDSLC_Components", strActionToProcess );

      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLDSLC_Components.jsp", "wSPLD.CancelSubregistrantLabelContent" );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "SaveAndReturn" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLDSLC_Components.jsp", "wSPLD.SaveSubregistrantLabelContent" );
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wSPLDSLC_Components" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSPLDSLC_Components.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSPLDSLC_Components.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<form name="wSPLDSLC_Components" id="wSPLDSLC_Components" method="post">
...
<%
   session.setAttribute( "ZeidonWindow", "wSPLDSLC_Components" );
   task.log().info( "After building the page setting ZeidonWindow: ------>>> " + "wSPLDSLC_Components" );
