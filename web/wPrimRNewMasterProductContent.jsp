      ...
      if ( nRC >= 0 ) // CursorResult.SET
      {
         ...
         try
         {
            if ( webMapping )
               ...
               wWebXfer.cursor( "Root" ).getAttribute( "AttemptStatementName" ).setValue( strMapValue, "" );
         }
...
else
{
   ...
   if ( VmlOperation.isValid( wWebXA ) )
   {
      ...
      wWebXA.cursor( "Root" ).getAttribute( "CurrentWindow" ).setValue( "NewMasterProductContent", "" );
   }
   ...
   else
   {
      ...
      else
      {
         ...
         if ( nRC >= 0 )
         {
            try
            {
               strErrorMapValue = wWebXfer.cursor( "Root" ).getAttribute( "AttemptStatementName" ).getString( "" );
            }
            catch (Exception e)
            {
               out.println("There is an error on EBContentName: " + e.getMessage());
               task.log().error( "*** Error on ctrl EBContentName", e );
            }
            ...
            task.log( ).debug( "Entity does not exist for EBContentName: " + "wWebXfer.Root" );
      }
   }
%>
...
<%
   ...
   task.log().info( "After building the page setting ZeidonWindow: ------>>> " + "wPrimRNewMasterProductContent" );
