...
<%-- wMLCAddUpdateKeywordSD   Generate Timestamp: 20160913084218863 --%>
                           ...
                           boolean webMapping )
{
   ...
   if ( VmlOperation.isValid( mMasLC ) )
   {
      ...
      nRC = mMasLC.cursor( "M_InsertTextKeywordSD" ).checkExistenceOfEntity( ).toInt();
      if ( nRC >= 0 ) // CursorResult.SET
      {
         ...
         try
         {
            if ( webMapping )
               ...
               mMasLC.cursor( "M_InsertTextKeywordSD" ).getAttribute( "Name" ).setValue( strMapValue, "" );
         }
      ...
      if ( nRC >= 0 )
      {
         ...
         try
         {
            if ( webMapping )
               ...
               mMasLC.cursor( "M_InsertTextKeywordSD" ).getAttribute( "Type" ).setValue( strMapValue, "" );
         }
         catch ( InvalidAttributeValueException e )
         {
            nMapError = -16;
      ...
      csrRC = vGridTmp.cursor( "M_InsertTextSD" ).setFirst(  );
      while ( csrRC.isSet() )
      {
         lEntityKey = vGridTmp.cursor( "M_InsertTextSD" ).getEntityKey( );
         ...
         if ( strMapValue != null ) 
         {
            try
            {
               if ( webMapping )
                  ...
                  vGridTmp.cursor( "M_InsertTextSD" ).getAttribute( "Text" ).setValue( strMapValue, "" );
            }
            catch ( InvalidAttributeValueException e )
            {
               nMapError = -16;
         ...
         csrRC = vGridTmp.cursor( "M_InsertTextSD" ).setNextContinue( );
      }
...
if ( strLastWindow.equals("wMLCAddUpdateKeywordSD") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wMLCAddUpdateKeywordSD strActionToProcess *** " + strActionToProcess );
task.log().info("*** wMLCAddUpdateKeywordSD LastWindow *** " + strLastWindow );
task.log().info("*** wMLCAddUpdateKeywordSD LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wMLCAddUpdateKeywordSD", strActionToProcess );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "AcceptNext" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wMLCAddUpdateKeywordSD", "wMLC.AcceptSD_KeywordGoNext" );
      nOptRC = wMLC.AcceptSD_KeywordGoNext( new zVIEW( vKZXMLPGO ) );
      if ( nOptRC == 2 )
      {
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "AcceptPrevious" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wMLCAddUpdateKeywordSD", "wMLC.AcceptSD_KeywordGoPrev" );
      nOptRC = wMLC.AcceptSD_KeywordGoPrev( new zVIEW( vKZXMLPGO ) );
      if ( nOptRC == 2 )
      {
         nRC = 2;  // do the "error" redirection
         session.setAttribute( "ZeidonError", "Y" );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "ADD_SD_KeywordTextItem" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wMLCAddUpdateKeywordSD", strActionToProcess );
      ...
      if ( VmlOperation.isValid( mMasLC ) )
      {
         ...
         csrRC = mMasLC.cursor( "M_InsertTextSD" ).setByEntityKey( lEKey );
         if ( !csrRC.isSet() )
         {
            boolean bFound = false;
            csrRCk = mMasLC.cursor( "M_InsertTextSD" ).setFirst( );
            while ( csrRCk.isSet() && !bFound )
            {
               lEKey = mMasLC.cursor( "M_InsertTextSD" ).getEntityKey( );
               ...
               if ( StringUtils.equals( strKey, strEntityKey ) )
               {
                  ...
                  csrRCk = mMasLC.cursor( "M_InsertTextSD" ).setNextContinue( );
            } // Grid
         }
      }
      ...
      try
      {
         EntityCursor cursor = mMasLC.cursor( "M_InsertTextSD" );
         cursor.createEntity( );
      ...
      {
         nRC = 2;
         VmlOperation.CreateMessage( task, "ADD_SD_KeywordTextItem", e.getMessage( ), "" );
         break;
      }
      // Next Window
      strNextJSP_Name = wMLC.SetWebRedirection( vKZXMLPGO, wMLC.zWAB_StayOnWindowWithRefresh, "", "" );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "ADD_SD_KeywordTextItemInit" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wMLCAddUpdateKeywordSD", strActionToProcess );
      ...
      {
         View mMasLCAuto = task.getViewByName( "mMasLC" );
         EntityCursor cursor = mMasLCAuto.cursor( "M_InsertTextSD" );
         cursor.createEntity( );
      ...
      {
         nRC = 2;
         VmlOperation.CreateMessage( task, "ADD_SD_KeywordTextItemInit", e.getMessage( ), "" );
         break;
      }
      // Next Window
      strNextJSP_Name = wMLC.SetWebRedirection( vKZXMLPGO, wMLC.zWAB_StayOnWindowWithRefresh, "", "" );
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wMLCAddUpdateKeywordSD" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wMLCAddUpdateKeywordSD.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wMLCAddUpdateKeywordSD.jsp") ) 
         {
            response.sendRedirect( strURL );
...
else
{
   ...
   if ( VmlOperation.isValid( wWebXA ) )
   {
      ...
      wWebXA.cursor( "Root" ).getAttribute( "CurrentWindow" ).setValue( "AddUpdateKeywordSD", "" );
   }
...
<title>AddUpdateKeywordSD</title>
...
<script language="JavaScript" type="text/javascript" src="./genjs/wMLCAddUpdateKeywordSD.js"></script>
...
<%
   csrRC = vKZXMLPGO.cursor( "DisableMenuOption" ).setFirst( "MenuOptionName", "PreviousKeyword" );
   if ( !csrRC.isSet() ) //if ( nRC < 0 )
   {
%>
       <li id="PreviousKeyword" name="PreviousKeyword"><a href="#"  onclick="AcceptPrevious()">Previous Keyword</a></li>
<%
   }
%>
...
<form name="wMLCAddUpdateKeywordSD" id="wMLCAddUpdateKeywordSD" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wMLC", "AddUpdateKeywordSD" );
   ...
   else
   {
      ...
      else
      {
         ...
         if ( nRC >= 0 )
         {
            try
            {
               strErrorMapValue = mMasLC.cursor( "M_InsertTextKeywordSD" ).getAttribute( "Name" ).getString( "" );
            }
            catch (Exception e)
            {
            ...
            task.log( ).debug( "M_InsertTextKeywordSD.Name: " + strErrorMapValue );
         }
         else
            task.log( ).debug( "Entity does not exist for DirectionsUseName1: " + "mMasLC.M_InsertTextKeywordSD" );
      }
   }
%>
...
<%
   ...
   if ( VmlOperation.isValid( mMasLC ) )
   {
      List<TableEntry> list = JspWebUtils.getTableDomainValues( mMasLC , "M_InsertTextKeywordSD", "Type", "" );

      nRC = mMasLC.cursor( "M_InsertTextKeywordSD" ).checkExistenceOfEntity( ).toInt();
      if ( nRC >= 0 )
      {
         strComboCurrentValue = mMasLC.cursor( "M_InsertTextKeywordSD" ).getAttribute( "Type" ).getString( "" );
...
<% /* PushBtn4:PushBtn */ %>
<button type="button" class="newbutton" name="PushBtn4" id="PushBtn4" value="" onclick="ADD_SD_KeywordTextItemInit( )" style="width:120px;height:26px;position:absolute;left:514px;top:4px;">Add Blank Entry</button>
...
try
{
   ...
   if ( VmlOperation.isValid( mMasLC ) )
   {
      ...
      csrRC2 = vGridM_InsertTextMarketing1.cursor( "M_InsertTextSD" ).setFirst(  );
      while ( csrRC2.isSet() )
      {
         strOdd = (iTableRowCnt % 2) != 0 ? " class='odd'" : "";
         iTableRowCnt++;

         lEntityKey = vGridM_InsertTextMarketing1.cursor( "M_InsertTextSD" ).getEntityKey( );
         strEntityKey = Long.toString( lEntityKey );
         strGridCtrlText11 = "";
         strGridCtrlText11ErrorColor = "";
         nRC = vGridM_InsertTextMarketing1.cursor( "M_InsertTextSD" ).checkExistenceOfEntity( ).toInt();
         if ( nRC >= 0 )
         {
            strTag = "GridCtrlText11::" + strEntityKey;
            ...
            {
               strGridCtrlText11 = vGridM_InsertTextMarketing1.cursor( "M_InsertTextSD" ).getAttribute( "Text" ).getString( "" );
...
<tr<%=strOdd%>>
   ...
   <td><input size="104" value="<%=strGridCtrlText11%>"<%=strGridCtrlText11ErrorColor%> name="GridCtrlText11::<%=strEntityKey%>" id="GridCtrlText11::<%=strEntityKey%>" ></td>
   <td nowrap><a href="#" style="display:block;width:100%;height:100%;text-decoration:none;" name="DeleteBtn1" onclick="ADD_SD_KeywordTextItem( this.id )" id="DeleteBtn1::<%=strEntityKey%>"><img src="./images/ePammsNew.png" alt="Add"></a></td>
...
<%
         csrRC2 = vGridM_InsertTextMarketing1.cursor( "M_InsertTextSD" ).setNextContinue( );
      }
...
<%
   session.setAttribute( "ZeidonWindow", "wMLCAddUpdateKeywordSD" );
