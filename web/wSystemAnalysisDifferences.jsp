                           ...
                           boolean webMapping )
{
   ...
   lMLCATgt = task.getViewByName( "lMLCATgt" );
   if ( VmlOperation.isValid( lMLCATgt ) )
   {
      // Grid: GridAnalysisResults
      iTableRowCnt = 0;

      // We are creating a temp view to the grid view so that if there are 
      // grids on the same window with the same view we do not mess up the 
      // entity positions. 
      vGridTmp = lMLCATgt.newView( );
      csrRC = vGridTmp.cursor( "ComparisonDifference" ).setFirst(  );
      while ( csrRC.isSet() )
      {
         lEntityKey = vGridTmp.cursor( "ComparisonDifference" ).getEntityKey( );
         strEntityKey = Long.toString( lEntityKey );
         iTableRowCnt++;

         csrRC = vGridTmp.cursor( "ComparisonDifference" ).setNextContinue( );
      }
...
if ( strLastWindow.equals("wSystemAnalysisDifferences") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSystemAnalysisDifferences strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSystemAnalysisDifferences LastWindow *** " + strLastWindow );
task.log().info("*** wSystemAnalysisDifferences LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   if ( task != null )
   {
      ...
      if ( strFeedback != "" )
      {
         ...
         wSystem.SaveFeedback( "mOrganiz", "wSystem", "AnalysisDifferences", strFeedback );
      }
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "smReturn" ) )
   {
      ...
      catch (Exception e)
      {
         ...
         nOptRC = 2;
         strVMLError = "<br><br>*** Error running Operation ReturnFromAnalysis: " + e.getMessage();
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "mProductManagement" ) )
   {
      ...
      {
         // Next Window
         strNextJSP_Name = wSystem.SetWebRedirection( vKZXMLPGO, wSystem.zWAB_StartTopWindow, "wStartUp", "AdminListPrimaryRegistrants" );
      }
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "mSubregistrants" ) )
   {
      ...
      {
         // Next Window
         strNextJSP_Name = wSystem.SetWebRedirection( vKZXMLPGO, wSystem.zWAB_StartTopWindow, "wStartUp", "AdminListSubregistrants" );
      }
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "mTrackingNotificationCompliance" ) )
   {
      ...
      {
         // Next Window
         strNextJSP_Name = wSystem.SetWebRedirection( vKZXMLPGO, wSystem.zWAB_StartTopWindow, "", "" );
      }
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "mAdministration" ) )
   {
      ...
      {
         // Next Window
         strNextJSP_Name = wSystem.SetWebRedirection( vKZXMLPGO, wSystem.zWAB_StartTopWindow, "wStartUp", "AdminUpdatePrimaryRegistrant" );
      }
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "mLogin" ) )
   {
      ...
      {
         // Next Window
         strNextJSP_Name = wSystem.SetWebRedirection( vKZXMLPGO, wSystem.zWAB_ResetTopWindow, "wStartUp", "UserLogin" );
      }
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "mTemplate" ) )
   {
      ...
      nRC = 0;
      wStartUp_Dialog wStartUp = new wStartUp_Dialog( vKZXMLPGO );
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSystemAnalysisDifferences.jsp", "wStartUp.Template" );
      try
      {
      ...
      {
         // Next Window
         strNextJSP_Name = wSystem.SetWebRedirection( vKZXMLPGO, wSystem.zWAB_ReplaceWindowWithModalWindow, "wTemplD", "TemplateList" );
      }
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----------------------------------->>> " + "wSystemAnalysisDifferences" );
         task.dropTask();
         task = null;
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSystemAnalysisDifferences.jsp") ) 
         {
            response.sendRedirect( strURL );
...
else
{
   ...
   if ( nOptRC == 2 )
   {
      ...
      strURLParameters = "?CallingPage=wSystemAnalysisDifferences.jsp" +
                         "&Message=" + strMessage +
                         "&DialogName=" + "wSystem" +
                         "&OperationName=" + "InitAnalysisDifferences";
...
<form name="wSystemAnalysisDifferences" id="wSystemAnalysisDifferences" method="post">
...
<% /* GroupBox1:GroupBox */ %>
<fieldset id="GroupBox1" name="GroupBox1"   style="size:absolute; width:854px; height:532px;">
<% /* GridAnalysisResults:Grid */ %>
<table  cols=3 style="position:absolute;top:20px;left:10px;width:826px;"  name="GridAnalysisResults" id="GridAnalysisResults">

<thead><tr>

   <th style="width:416px;">Title</th>
   <th style="width:186px;">New MLC Value</th>
   <th style="width:186px;">Prior SLC Value</th>
...
try
{
   ...
   if ( VmlOperation.isValid( lMLCATgt ) )
   {
      ...
      String strTag;
      String strGridTitle;
      String strGridNewMLC_Value;
      String strGridPriorSLC_Value;
      
      View vGridAnalysisResults;
      vGridAnalysisResults = lMLCATgt.newView( );
      csrRC2 = vGridAnalysisResults.cursor( "ComparisonDifference" ).setFirst(  );
      while ( csrRC2.isSet() )
      {
         ...
         if ( StringUtils.isBlank( strGridNewMLC_Value ) )
            strGridNewMLC_Value = "&nbsp";
         ...
         if ( StringUtils.isBlank( strGridPriorSLC_Value ) )
            strGridPriorSLC_Value = "&nbsp";

%>

<tr<%=strOdd%>>

   <td nowrap style="width:416px;"><%=strGridTitle%></td>
   <td nowrap style="width:186px;"><%=strGridNewMLC_Value%></td>
   <td nowrap style="width:186px;"><%=strGridPriorSLC_Value%></td>

</tr>

<%
         csrRC2 = vGridAnalysisResults.cursor( "ComparisonDifference" ).setNextContinue( );
      }
      vGridAnalysisResults.drop( );
   }
}
catch (Exception e)
{
