...
<%-- wMLCAddUpdateSectionKeywordDU   Generate Timestamp: 20160913084218940 --%>
                           ...
                           boolean webMapping )
{
   ...
   if ( VmlOperation.isValid( mMasLC ) )
   {
      ...
      nRC = mMasLC.cursor( "M_InsertTextKeywordSectionDU" ).checkExistenceOfEntity( ).toInt();
      if ( nRC >= 0 ) // CursorResult.SET
      {
         ...
         try
         {
            if ( webMapping )
               ...
               mMasLC.cursor( "M_InsertTextKeywordSectionDU" ).getAttribute( "Name" ).setValue( strMapValue, "" );
         }
      ...
      if ( nRC >= 0 )
      {
         ...
         try
         {
            if ( webMapping )
               ...
               mMasLC.cursor( "M_InsertTextKeywordSectionDU" ).getAttribute( "Type" ).setValue( strMapValue, "" );
         }
         catch ( InvalidAttributeValueException e )
         {
            nMapError = -16;
      ...
      csrRC = vGridTmp.cursor( "M_InsertTextSectionDU" ).setFirst(  );
      while ( csrRC.isSet() )
      {
         lEntityKey = vGridTmp.cursor( "M_InsertTextSectionDU" ).getEntityKey( );
         ...
         if ( strMapValue != null ) 
         {
            try
            {
               if ( webMapping )
                  ...
                  vGridTmp.cursor( "M_InsertTextSectionDU" ).getAttribute( "Text" ).setValue( strMapValue, "" );
            }
            catch ( InvalidAttributeValueException e )
            {
               nMapError = -16;
         ...
         csrRC = vGridTmp.cursor( "M_InsertTextSectionDU" ).setNextContinue( );
      }
...
if ( strLastWindow.equals("wMLCAddUpdateSectionKeywordDU") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wMLCAddUpdateSectionKeywordDU strActionToProcess *** " + strActionToProcess );
task.log().info("*** wMLCAddUpdateSectionKeywordDU LastWindow *** " + strLastWindow );
task.log().info("*** wMLCAddUpdateSectionKeywordDU LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wMLCAddUpdateSectionKeywordDU", strActionToProcess );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "AddNewKeyword" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wMLCAddUpdateSectionKeywordDU", strActionToProcess );
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wMLCAddUpdateSectionKeywordDU", "wMLC.AcceptAddNewDU_SectionKeyword" );
      nOptRC = wMLC.AcceptAddNewDU_SectionKeyword( new zVIEW( vKZXMLPGO ) );
      if ( nOptRC == 2 )
      {
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "ADD_DU_KeywordTextItem" ) )
   {
      ...
      if ( VmlOperation.isValid( mMasLC ) )
      {
         ...
         csrRC = mMasLC.cursor( "M_InsertTextSectionDU" ).setByEntityKey( lEKey );
         if ( !csrRC.isSet() )
         {
            boolean bFound = false;
            csrRCk = mMasLC.cursor( "M_InsertTextSectionDU" ).setFirst( );
            while ( csrRCk.isSet() && !bFound )
            {
               lEKey = mMasLC.cursor( "M_InsertTextSectionDU" ).getEntityKey( );
               ...
               if ( StringUtils.equals( strKey, strEntityKey ) )
               {
                  ...
                  csrRCk = mMasLC.cursor( "M_InsertTextSectionDU" ).setNextContinue( );
            } // Grid
         }
      }
      ...
      try
      {
         EntityCursor cursor = mMasLC.cursor( "M_InsertTextSectionDU" );
         cursor.createEntity( );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "ADD_DU_KeywordTextItemInit" ) )
   {
      ...
      {
         View mMasLCAuto = task.getViewByName( "mMasLC" );
         EntityCursor cursor = mMasLCAuto.cursor( "M_InsertTextSectionDU" );
         cursor.createEntity( );
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wMLCAddUpdateSectionKeywordDU" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wMLCAddUpdateSectionKeywordDU.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wMLCAddUpdateSectionKeywordDU.jsp") ) 
         {
            response.sendRedirect( strURL );
...
else
{
   ...
   if ( VmlOperation.isValid( wWebXA ) )
   {
      ...
      wWebXA.cursor( "Root" ).getAttribute( "CurrentWindow" ).setValue( "AddUpdateSectionKeywordDU", "" );
   }
...
<title>Directions For Use Keyword Section</title>
...
<script language="JavaScript" type="text/javascript" src="./genjs/wMLCAddUpdateSectionKeywordDU.js"></script>
...
<%
   ...
   {
%>
       <li id="AddNew" name="AddNew"><a href="#"  onclick="AddNewKeyword()">Add New</a></li>
<%
   }
%>
...
<form name="wMLCAddUpdateSectionKeywordDU" id="wMLCAddUpdateSectionKeywordDU" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wMLC", "AddUpdateSectionKeywordDU" );
   ...
   else
   {
      ...
      else
      {
         ...
         if ( nRC >= 0 )
         {
            try
            {
               strErrorMapValue = mMasLC.cursor( "M_InsertTextKeywordSectionDU" ).getAttribute( "Name" ).getString( "" );
            }
            catch (Exception e)
            {
            ...
            task.log( ).debug( "M_InsertTextKeywordSectionDU.Name: " + strErrorMapValue );
         }
         else
            task.log( ).debug( "Entity does not exist for DirectionsUseName: " + "mMasLC.M_InsertTextKeywordSectionDU" );
      }
   }
%>
...
<%
   ...
   if ( VmlOperation.isValid( mMasLC ) )
   {
      List<TableEntry> list = JspWebUtils.getTableDomainValues( mMasLC , "M_InsertTextKeywordSectionDU", "Type", "" );

      nRC = mMasLC.cursor( "M_InsertTextKeywordSectionDU" ).checkExistenceOfEntity( ).toInt();
      if ( nRC >= 0 )
      {
         strComboCurrentValue = mMasLC.cursor( "M_InsertTextKeywordSectionDU" ).getAttribute( "Type" ).getString( "" );
...
<% /* GridM_InsertTextTitle:Grid */ %>
<table  cols=3 style=""  name="GridM_InsertTextTitle" id="GridM_InsertTextTitle">
...
try
{
   ...
   if ( VmlOperation.isValid( mMasLC ) )
   {
      ...
      View vGridM_InsertTextTitle;
      vGridM_InsertTextTitle = mMasLC.newView( );
      csrRC2 = vGridM_InsertTextTitle.cursor( "M_InsertTextSectionDU" ).setFirst(  );
      while ( csrRC2.isSet() )
      {
         strOdd = (iTableRowCnt % 2) != 0 ? " class='odd'" : "";
         iTableRowCnt++;

         lEntityKey = vGridM_InsertTextTitle.cursor( "M_InsertTextSectionDU" ).getEntityKey( );
         strEntityKey = Long.toString( lEntityKey );
         strGridCtrlText = "";
         strGridCtrlTextErrorColor = "";
         nRC = vGridM_InsertTextTitle.cursor( "M_InsertTextSectionDU" ).checkExistenceOfEntity( ).toInt();
         if ( nRC >= 0 )
         {
            strTag = "GridCtrlText::" + strEntityKey;
            ...
            {
               strGridCtrlText = vGridM_InsertTextTitle.cursor( "M_InsertTextSectionDU" ).getAttribute( "Text" ).getString( "" );
...
<%
         csrRC2 = vGridM_InsertTextTitle.cursor( "M_InsertTextSectionDU" ).setNextContinue( );
      }
      vGridM_InsertTextTitle.drop( );
   }
}
catch (Exception e)
{
...
<%
   session.setAttribute( "ZeidonWindow", "wMLCAddUpdateSectionKeywordDU" );
