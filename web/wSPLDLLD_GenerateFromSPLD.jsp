...
<%-- wSPLDLLD_GenerateFromSPLD   Generate Timestamp: 20160620105929388 --%>
      ...
      if ( nRC >= 0 ) // CursorResult.SET
      {
         ...
         try
         {
            if ( webMapping )
               ...
               mSPLDef.cursor( "SubregPhysicalLabelDef" ).getAttribute( "wSavedLLD_Name" ).setValue( strMapValue, "" );
         }
...
if ( strLastWindow.equals("wSPLDLLD_GenerateFromSPLD") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSPLDLLD_GenerateFromSPLD strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSPLDLLD_GenerateFromSPLD LastWindow *** " + strLastWindow );
task.log().info("*** wSPLDLLD_GenerateFromSPLD LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSPLDLLD_GenerateFromSPLD", strActionToProcess );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "GenerateAndReturn" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLDLLD_GenerateFromSPLD", "wSPLD.GENERATE_LLD_FromSPLD" );
      nOptRC = wSPLD.GENERATE_LLD_FromSPLD( new zVIEW( vKZXMLPGO ) );
      if ( nOptRC == 2 )
      {
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wSPLDLLD_GenerateFromSPLD" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSPLDLLD_GenerateFromSPLD.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSPLDLLD_GenerateFromSPLD.jsp") ) 
         {
            response.sendRedirect( strURL );
...
else
{
   ...
   if ( VmlOperation.isValid( wWebXA ) )
   {
      ...
      wWebXA.cursor( "Root" ).getAttribute( "CurrentWindow" ).setValue( "LLD_GenerateFromSPLD", "" );
   }
...
<title>Generate LLD From SPLD</title>
...
<form name="wSPLDLLD_GenerateFromSPLD" id="wSPLDLLD_GenerateFromSPLD" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSPLD", "LLD_GenerateFromSPLD" );
   ...
   else
   {
      ...
      else
      {
         ...
         if ( nRC >= 0 )
         {
            ...
            task.log( ).debug( "Entity does not exist for EditBox2: " + "mSPLDef.SubregPhysicalLabelDef" );
      }
   }
%>
