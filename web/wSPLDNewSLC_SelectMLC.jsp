...
if ( strLastWindow.equals("wSPLDNewSLC_SelectMLC") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSPLDNewSLC_SelectMLC strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSPLDNewSLC_SelectMLC LastWindow *** " + strLastWindow );
task.log().info("*** wSPLDNewSLC_SelectMLC LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSPLDNewSLC_SelectMLC", strActionToProcess );
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLDNewSLC_SelectMLC.jsp", "wSPLD.SELECT_MLC_ForNewSLC" );
         nOptRC = wSPLD.SELECT_MLC_ForNewSLC( new zVIEW( vKZXMLPGO ) );
      if ( nOptRC == 2 )
      {
      ...
      {
         // Next Window
         strNextJSP_Name = wSPLD.SetWebRedirection( vKZXMLPGO, wSPLD.zWAB_ReplaceWindowWithModalWindow, "wSPLD", "SLC_ComponentsUpdate" );
      }
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wSPLDNewSLC_SelectMLC" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSPLDNewSLC_SelectMLC.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSPLDNewSLC_SelectMLC.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSPLDNewSLC_SelectMLC.js"></script>
...
<form name="wSPLDNewSLC_SelectMLC" id="wSPLDNewSLC_SelectMLC" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSPLD", "NewSLC_SelectMLC" );
...
<%
   session.setAttribute( "ZeidonWindow", "wSPLDNewSLC_SelectMLC" );
