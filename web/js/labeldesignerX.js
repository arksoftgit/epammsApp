...
// alert( "Session objRV: " + objRV.toString() );
   ...
   function getParentagePathKeys( element ) {
      ...
      var path = "LLD_Page." + $("#page" + g_currentPage).attr( "id" ) + ";";
      var $el;
   ...
   function saveLabelCallback( jsonZeidon, action, name ) {
      if ( name === "mSPLDef" ) {
         console.log( "JSON LLD saved!!!" );
         g_updatedLLD = false;
   ...
   function saveLLD() {
      if ( g_updatedLLD ) {
         g_loadedLLD = "mSPLDef";
         ConvertWysiwygLabelDesignToZeidonJson( "saveLabel", g_loadedLLD, saveLabelCallback );
      }
   }
   ...
   $("body").on( "dblclick", ".canvas-element", function(e) {
      ...
      saveLLD();
      var parentagePath = getParentagePathKeys( this );
      GOTO_UpdateBlockComponent( parentagePath );
      return false;
   });
   ...
   // hoverClass: "ui-state-active",  partially highlights page ... not so good
      ...
      // console.log( ".page, .block-element drop" );
         ...
         // $canvasElement.append( "<h5 class='ui-widget-header'></h5>" );
            ...
            $canvasElement.children( ".ui-icon" ).css( "z-index", "" );
            $canvasParent = $canvas;
...
/*
   ...
   // console.log( "\nJsonLabel: " + jsonLabel );
      ...
      // $id("zFormattedJsonLabel").innerHTML = "<PRE class='CodeContainer'>" + formattedHtml + "</PRE>";
         ...
         var jqxhr = $.post( url, FormatToJsonLLD_Callback ) // the function to run if the request succeeds
            .done( function() {
...
*/
   ...
   //   status - contains the status of the request ("success", "notmodified", "error", "timeout", or "parsererror")
   //   xhr - contains the XMLHttpRequest object
/* function ConvertWysiwygLabelDesignToZeidonJson_Callback( jsonZeidon, status, xhr ) {
      if ( xhr.readyState === 4 ) {
         if ( xhr.status === 200 ) {
         // parseMessages( req.responseXML );
            console.log( "JSON Zeidon: " + jsonZeidon );
...
*/
/*
   function ConvertWysiwygLabelDesignToZeidonJson( action, name, callback_func ) {
   ...
   // console.log( "\nJsonLabel: " + jsonLabel );
      ...
      $.ajax({
         ...
         dataType : 'json',
         success : callback_func( jsonZeidon, action, name )
      });
   }
*/
         ...
         $.ajax({ url: url,
               ...
               // beforeSend - callback function that is executed before the request is sent
                  success: function( data, textStatus, jqXHR ) {
                              ...
                              callback_func( data, action, name );
                           },
      ...
      // console.log( "resizeable containment: #" + $root.attr( "id" ) );
         $element = $(div).text( tag )
            ...
            //.append( "<h5 class=\"ui-widget-header\"></h5>" );
              .append( "<h5></h5>" );
         $element.children( ".ui-icon" ).css( "z-index", "" );
