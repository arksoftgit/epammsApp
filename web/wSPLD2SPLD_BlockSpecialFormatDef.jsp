...
<%-- wSPLD2SPLD_BlockSpecialFormatDef --%>
      ...
      if ( nRC >= 0 )
      {
         ...
         try
         {
            if ( webMapping )
               ...
               mSPLDef.cursor( "LLD_SpecialSectionAttrBlock" ).setAttribute( "Color", strMapValue, "" );
         }
         catch ( InvalidAttributeValueException e )
         {
            nMapError = -16;
...
if ( strLastWindow.equals("wSPLD2SPLD_BlockSpecialFormatDef") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSPLD2SPLD_BlockSpecialFormatDef strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSPLD2SPLD_BlockSpecialFormatDef LastWindow *** " + strLastWindow );
task.log().info("*** wSPLD2SPLD_BlockSpecialFormatDef LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSPLD2SPLD_BlockSpecialFormatDef", strActionToProcess );
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLD2SPLD_BlockSpecialFormatDef.jsp", "wSPLD2.ACCEPT_BlockSpecialFormat" );
      try
      {
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "CANCEL_BlockComponent" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLD2SPLD_BlockSpecialFormatDef.jsp", "wSPLD2.CANCEL_BlockSpecialFormat" );
      try
      {
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----------------------------------->>> " + "wSPLD2SPLD_BlockSpecialFormatDef" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSPLD2SPLD_BlockSpecialFormatDef.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSPLD2SPLD_BlockSpecialFormatDef.jsp") ) 
         {
            response.sendRedirect( strURL );
...
{
   VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLD2SPLD_BlockSpecialFormatDef.jsp", "wSPLD2.PostbuildBlockSpecialFormatDef" );
   ...
   if ( nOptRC == 2 )
   {
      ...
      strURLParameters = "?CallingPage=wSPLD2SPLD_BlockSpecialFormatDef.jsp" +
                         "&Message=" + strMessage +
                         "&DialogName=" + "wSPLD2" +
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSPLD2SPLD_BlockSpecialFormatDef.js"></script>
...
<form name="wSPLD2SPLD_BlockSpecialFormatDef" id="wSPLD2SPLD_BlockSpecialFormatDef" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSPLD2", "SPLD_BlockSpecialFormatDef" );
...
<%
   ...
   if ( VmlOperation.isValid( mSPLDef ) )
   {
      List<TableEntry> list = JspWebUtils.getTableDomainValues( mSPLDef , "LLD_SpecialSectionAttrBlock", "Color", "" );

      nRC = mSPLDef.cursor( "LLD_SpecialSectionAttrBlock" ).checkExistenceOfEntity( ).toInt();
      if ( nRC >= 0 )
      {
         strComboCurrentValue = mSPLDef.cursor( "LLD_SpecialSectionAttrBlock" ).getStringFromAttribute( "Color", "" );
         if ( strComboCurrentValue == null )
            strComboCurrentValue = "";
      }
      else
...
<%
   session.setAttribute( "ZeidonWindow", "wSPLD2SPLD_BlockSpecialFormatDef" );
