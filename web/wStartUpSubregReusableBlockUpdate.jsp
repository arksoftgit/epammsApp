      ...
      if ( nRC >= 0 ) // CursorResult.SET
      {
         ...
         try
         {
            if ( webMapping )
               ...
               mSubreg.cursor( "ReusableBlockDefinition" ).getAttribute( "Description" ).setValue( strMapValue, "" );
         }
         catch ( InvalidAttributeValueException e )
         {
            nMapError = -16;
...
if ( strLastWindow.equals("wStartUpSubregReusableBlockUpdate") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wStartUpSubregReusableBlockUpdate strActionToProcess *** " + strActionToProcess );
task.log().info("*** wStartUpSubregReusableBlockUpdate LastWindow *** " + strLastWindow );
task.log().info("*** wStartUpSubregReusableBlockUpdate LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wStartUpSubregReusableBlockUpdate", strActionToProcess );
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wStartUpSubregReusableBlockUpdate" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wStartUpSubregReusableBlockUpdate.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wStartUpSubregReusableBlockUpdate.jsp") ) 
         {
            response.sendRedirect( strURL );
...
else
{
   ...
   if ( nOptRC == 2 )
   {
      ...
      strURLParameters = "?CallingPage=wStartUpSubregReusableBlockUpdate.jsp" +
                         "&Message=" + strMessage +
                         "&DialogName=" + "wStartUp" +
   ...
   if ( VmlOperation.isValid( wWebXA ) )
   {
      ...
      wWebXA.cursor( "Root" ).getAttribute( "CurrentWindow" ).setValue( "SubregReusableBlockUpdate", "" );
   }
...
<form name="wStartUpSubregReusableBlockUpdate" id="wStartUpSubregReusableBlockUpdate" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wStartUp", "SubregReusableBlockUpdate" );
