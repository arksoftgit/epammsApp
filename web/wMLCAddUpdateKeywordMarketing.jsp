...
<%-- wMLCAddUpdateKeywordMarketing   Generate Timestamp: 20160531205222600 --%>
                           ...
                           boolean webMapping )
{
   ...
   if ( VmlOperation.isValid( mMasLC ) )
   {
      ...
      if ( nRC >= 0 )
      {
         ...
         try
         {
            if ( webMapping )
               ...
               mMasLC.cursor( "M_InsertTextKeywordMarketing" ).getAttribute( "Type" ).setValue( strMapValue, "" );
         }
         catch ( InvalidAttributeValueException e )
         {
            nMapError = -16;
      ...
      csrRC = vGridTmp.cursor( "M_InsertTextMarketing" ).setFirst(  );
      while ( csrRC.isSet() )
      {
         ...
         if ( strMapValue != null ) 
         {
            try
            {
               if ( webMapping )
                  ...
                  vGridTmp.cursor( "M_InsertTextMarketing" ).getAttribute( "Text" ).setValue( strMapValue, "" );
            }
            catch ( InvalidAttributeValueException e )
            {
               nMapError = -16;
...
if ( strLastWindow.equals("wMLCAddUpdateKeywordMarketing") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wMLCAddUpdateKeywordMarketing strActionToProcess *** " + strActionToProcess );
task.log().info("*** wMLCAddUpdateKeywordMarketing LastWindow *** " + strLastWindow );
task.log().info("*** wMLCAddUpdateKeywordMarketing LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wMLCAddUpdateKeywordMarketing", strActionToProcess );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "ADD_MarketingKeywordTextItem" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wMLCAddUpdateKeywordMarketing", strActionToProcess );
      ...
      if ( VmlOperation.isValid( mMasLC ) )
      {
         ...
         if ( !csrRC.isSet() )
         {
            ...
            csrRCk = mMasLC.cursor( "M_InsertTextMarketing" ).setFirst( );
      ...
      try
      {
         EntityCursor cursor = mMasLC.cursor( "M_InsertTextMarketing" );
         cursor.createEntity( );
      ...
      {
         nRC = 2;
         VmlOperation.CreateMessage( task, "ADD_MarketingKeywordTextItem", e.getMessage( ), "" );
         break;
      }
      // Next Window
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "ADD_MarketingKeywordTextItemInit" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wMLCAddUpdateKeywordMarketing", strActionToProcess );
      ...
      {
         View mMasLCAuto = task.getViewByName( "mMasLC" );
         EntityCursor cursor = mMasLCAuto.cursor( "M_InsertTextMarketing" );
         cursor.createEntity( );
      ...
      {
         nRC = 2;
         VmlOperation.CreateMessage( task, "ADD_MarketingKeywordTextItemInit", e.getMessage( ), "" );
         break;
      }
      // Next Window
      strNextJSP_Name = wMLC.SetWebRedirection( vKZXMLPGO, wMLC.zWAB_StayOnWindowWithRefresh, "", "" );
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wMLCAddUpdateKeywordMarketing" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wMLCAddUpdateKeywordMarketing.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wMLCAddUpdateKeywordMarketing.jsp") ) 
         {
            response.sendRedirect( strURL );
...
else
{
   ...
   if ( VmlOperation.isValid( wWebXA ) )
   {
      ...
      wWebXA.cursor( "Root" ).getAttribute( "CurrentWindow" ).setValue( "AddUpdateKeywordMarketing", "" );
   }
...
<title>AddUpdateKeywordMarketing</title>
...
<script language="JavaScript" type="text/javascript" src="./genjs/wMLCAddUpdateKeywordMarketing.js"></script>
...
<form name="wMLCAddUpdateKeywordMarketing" id="wMLCAddUpdateKeywordMarketing" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wMLC", "AddUpdateKeywordMarketing" );
...
<div id="GBAddSurfacesList" name="GBAddSurfacesList" style="float:left;width:426px;" >

<table cols=2 style="width:426px;"  class="grouptable">
...
<%
   ...
   if ( VmlOperation.isValid( mMasLC ) )
   {
      List<TableEntry> list = JspWebUtils.getTableDomainValues( mMasLC , "M_InsertTextKeywordMarketing", "Type", "" );

      nRC = mMasLC.cursor( "M_InsertTextKeywordMarketing" ).checkExistenceOfEntity( ).toInt();
      if ( nRC >= 0 )
      {
         strComboCurrentValue = mMasLC.cursor( "M_InsertTextKeywordMarketing" ).getAttribute( "Type" ).getString( "" );
...
<div id="GroupBox1" name="GroupBox1" style="width:726px;float:left;">  <!-- GroupBox1 --> 
...
<div id="GroupBox7" name="GroupBox7"   style="float:left;position:relative; width:678px; height:30px;">  <!-- GroupBox7 --> 
...
<% /* PushBtn:PushBtn */ %>
<button type="button" class="newbutton" name="PushBtn" id="PushBtn" value="" onclick="ADD_MarketingKeywordTextItemInit( )" style="width:120px;height:26px;position:absolute;left:490px;top:4px;">Add Blank Entry</button>
...
try
{
   ...
   if ( VmlOperation.isValid( mMasLC ) )
   {
      ...
      csrRC2 = vGridM_InsertTextMarketing1.cursor( "M_InsertTextMarketing" ).setFirst(  );
      while ( csrRC2.isSet() )
      {
         strOdd = (iTableRowCnt % 2) != 0 ? " class='odd'" : "";
         iTableRowCnt++;

         lEntityKey = vGridM_InsertTextMarketing1.cursor( "M_InsertTextMarketing" ).getEntityKey( );
         strEntityKey = Long.toString( lEntityKey );
         strGridCtrlText = "";
         strGridCtrlTextErrorColor = "";
         nRC = vGridM_InsertTextMarketing1.cursor( "M_InsertTextMarketing" ).checkExistenceOfEntity( ).toInt();
         if ( nRC >= 0 )
         {
            strTag = "GridCtrlText::" + strEntityKey;
            ...
            {
               strGridCtrlText = vGridM_InsertTextMarketing1.cursor( "M_InsertTextMarketing" ).getAttribute( "Text" ).getString( "" );
...
<tr<%=strOdd%>>
   ...
   <td nowrap><a href="#" style="display:block;width:100%;height:100%;text-decoration:none;" name="AddBtn" onclick="ADD_MarketingKeywordTextItem( this.id )" id="AddBtn::<%=strEntityKey%>"><img src="./images/ePammsNew.png" alt="Add"></a></td>
...
<%
         csrRC2 = vGridM_InsertTextMarketing1.cursor( "M_InsertTextMarketing" ).setNextContinue( );
      }
