...
<%-- wSPLDSPLD_StorageDisposalSection   Generate Timestamp: 20170509200336885 --%>
...
if ( strLastWindow.equals("wSPLDSPLD_StorageDisposalSection") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSPLDSPLD_StorageDisposalSection strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSPLDSPLD_StorageDisposalSection LastWindow *** " + strLastWindow );
task.log().info("*** wSPLDSPLD_StorageDisposalSection LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSPLDSPLD_StorageDisposalSection", strActionToProcess );
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wSPLDSPLD_StorageDisposalSection" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSPLDSPLD_StorageDisposalSection.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSPLDSPLD_StorageDisposalSection.jsp") ) 
         {
            response.sendRedirect( strURL );
...
else
{
   ...
   if ( VmlOperation.isValid( wWebXA ) )
   {
      ...
      wWebXA.cursor( "Root" ).getAttribute( "CurrentWindow" ).setValue( "SPLD_StorageDisposalSection", "" );
   }
...
<title>SPLD Storage & Disposal Section</title>
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSPLDSPLD_StorageDisposalSection.js"></script>
...
<form name="wSPLDSPLD_StorageDisposalSection" id="wSPLDSPLD_StorageDisposalSection" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSPLD", "SPLD_StorageDisposalSection" );
...
<label class="groupbox"  id="StorageDisposalSections:" name="StorageDisposalSections:" style="width:202px;height:16px;position:absolute;left:6px;top:12px;">Storage and Disposal Section:</label>
...
<% strTextDisplayValue = "";
   ...
   else
   {
      ...
      try
      {
         strTextDisplayValue = mSPLDef.cursor( "SPLD_StorageDisposalSection" ).getAttribute( "Name" ).getString( "" );
      }
      catch (Exception e)
      {
...
<%
   ...
   else
   {
      ...
      else
      {
         ...
         if ( nRC >= 0 )
         {
            strErrorMapValue = mSPLDef.cursor( "SPLD_StorageDisposalSection" ).getAttribute( "Title" ).getString( "" );
            if ( strErrorMapValue == null )
               strErrorMapValue = "";

            task.log( ).info( "SPLD_StorageDisposalSection.Title: " + strErrorMapValue );
         }
         else
            task.log( ).info( "Entity does not exist for Title: " + "mSPLDef.SPLD_StorageDisposalSection" );
      }
   }
%>
...
<%
   ...
   else
   {
      ...
      else
      {
         ...
         if ( nRC >= 0 )
         {
            strErrorMapValue = mSPLDef.cursor( "SPLD_StorageDisposalSection" ).getAttribute( "Subtitle" ).getString( "" );
            if ( strErrorMapValue == null )
               strErrorMapValue = "";

            task.log( ).info( "SPLD_StorageDisposalSection.Subtitle: " + strErrorMapValue );
         }
         else
            task.log( ).info( "Entity does not exist for Text: " + "mSPLDef.SPLD_StorageDisposalSection" );
      }
   }
%>
...
<%
   ...
   else
   {
      nRC = mSPLDef.cursor( "SPLD_StorageDisposalSection" ).checkExistenceOfEntity( ).toInt();
      if ( nRC >= 0 )
         strRadioGroupValue = mSPLDef.cursor( "SPLD_StorageDisposalSection" ).getAttribute( "ExclusiveStatements" ).getString( );
   }
...
try
{
   ...
   if ( VmlOperation.isValid( mSPLDef ) )
   {
      ...
      while ( csrRC2.isSet() )
      {
         ...
         if ( nRC >= 0 )
         {
            strGridEditStorDispText = vGridStorDisp.cursor( "SPLD_StorageDisposalStatement" ).getAttribute( "dSD_TitleTextKey" ).getString( "" );
...
<%
   session.setAttribute( "ZeidonWindow", "wSPLDSPLD_StorageDisposalSection" );
