...
<%-- wSPLDStorageDisposalSubStatement   Generate Timestamp: 20170509200313965 --%>
...
if ( strLastWindow.equals("wSPLDStorageDisposalSubStatement") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSPLDStorageDisposalSubStatement strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSPLDStorageDisposalSubStatement LastWindow *** " + strLastWindow );
task.log().info("*** wSPLDStorageDisposalSubStatement LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSPLDStorageDisposalSubStatement", strActionToProcess );

      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLDStorageDisposalSubStatement", "wSPLD.RETURN_StorageDisposalSubStmt" );
      nOptRC = wSPLD.RETURN_StorageDisposalSubStmt( new zVIEW( vKZXMLPGO ) );
      if ( nOptRC == 2 )
      {
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wSPLDStorageDisposalSubStatement" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSPLDStorageDisposalSubStatement.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSPLDStorageDisposalSubStatement.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<title>SPLD Storage & Disposal Sub-Statement</title>
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSPLDStorageDisposalSubStatement.js"></script>
...
<form name="wSPLDStorageDisposalSubStatement" id="wSPLDStorageDisposalSubStatement" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSPLD", "StorageDisposalSubStatement" );
   ...
   else
   {
      ...
      else
      {
         ...
         if ( nRC >= 0 )
         {
            ...
            task.log( ).debug( "Entity does not exist for MLEdit3: " + "mSPLDef.SPLD_StorageDisposalStatement" );
      }
   }
%>
   ...
   else
   {
      ...
      else
      {
         ...
         if ( nRC >= 0 )
         {
            ...
            task.log( ).debug( "Entity does not exist for MLEdit2: " + "mSPLDef.SPLD_StorageDisposalStatement" );
      }
   }
%>
...
try
{
   ...
   if ( VmlOperation.isValid( mSPLDef ) )
   {
      ...
      csrRC2 = vGrid4.cursor( "SPLD_InsertTextKeywordSD" ).setFirst(  );
      while ( csrRC2.isSet() )
      {
         strOdd = (iTableRowCnt % 2) != 0 ? " class='odd'" : "";
         iTableRowCnt++;

         lEntityKey = vGrid4.cursor( "SPLD_InsertTextKeywordSD" ).getEntityKey( );
         strEntityKey = Long.toString( lEntityKey );
         strGridCheckCtl = "";
         nRC = vGrid4.cursor( "SPLD_InsertTextKeywordSD" ).checkExistenceOfEntity( ).toInt();
         if ( nRC >= 0 )
         {
            strGridCheckCtl = vGrid4.cursor( "SPLD_InsertTextKeywordSD" ).getAttribute( "Selected" ).getString( "" );
         ...
         if ( nRC >= 0 )
         {
            strGridEditCtl4 = vGrid4.cursor( "SPLD_InsertTextKeywordSD" ).getAttribute( "Name" ).getString( "" );
         ...
         if ( nRC >= 0 )
         {
            strGridEditCtl5 = vGrid4.cursor( "SPLD_InsertTextKeywordSD" ).getAttribute( "dSD_KeywordText" ).getString( "" );
...
<%
         csrRC2 = vGrid4.cursor( "SPLD_InsertTextKeywordSD" ).setNextContinue( );
      }
      vGrid4.drop( );
   }
}
...
<%
   session.setAttribute( "ZeidonWindow", "wSPLDStorageDisposalSubStatement" );
