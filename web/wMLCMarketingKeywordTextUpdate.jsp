...
<%-- wMLCMarketingKeywordTextUpdate --%>
...
if ( strLastWindow.equals("wMLCMarketingKeywordTextUpdate") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wMLCMarketingKeywordTextUpdate strActionToProcess *** " + strActionToProcess );
task.log().info("*** wMLCMarketingKeywordTextUpdate LastWindow *** " + strLastWindow );
task.log().info("*** wMLCMarketingKeywordTextUpdate LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "AcceptMarketingKeywordText" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wMLCMarketingKeywordTextUpdate", strActionToProcess );
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wMLCMarketingKeywordTextUpdate" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wMLCMarketingKeywordTextUpdate.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wMLCMarketingKeywordTextUpdate.jsp") ) 
         {
            response.sendRedirect( strURL );
...
else
{
   ...
   if ( VmlOperation.isValid( wWebXA ) )
   {
      ...
      wWebXA.cursor( "Root" ).getAttribute( "CurrentWindow" ).setValue( "MarketingKeywordTextUpdate", "" );
   }
...
<title>MarketingKeywordTextUpdate</title>
...
<script language="JavaScript" type="text/javascript" src="./genjs/wMLCMarketingKeywordTextUpdate.js"></script>
...
<%
   ...
   {
%>
       <li id="AcceptAndReturn" name="AcceptAndReturn"><a href="#"  onclick="AcceptMarketingKeywordText()">Accept & Return</a></li>
<%
   }
%>
...
<form name="wMLCMarketingKeywordTextUpdate" id="wMLCMarketingKeywordTextUpdate" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wMLC", "MarketingKeywordTextUpdate" );
...
<label class="groupbox"  id="OrganismClaimsStatements3" name="OrganismClaimsStatements3" style="width:324px;height:16px;position:absolute;left:6px;top:12px;">Marketing Keyword & Keyword Text Update</label>
...
<span  id="DirectionsUseTitle:" name="DirectionsUseTitle:" style="width:72px;height:16px;">Keyword:</span>
...
<span  id="Text2" name="Text2" style="width:72px;height:16px;">Text:</span>
...
<div style="clear:both;"></div>  <!-- Moving to a new line, so do a clear -->
 ...
 <!-- This is added as a line spacer -->
<div style="height:904px;width:100px;"></div>
...
<%
   session.setAttribute( "ZeidonWindow", "wMLCMarketingKeywordTextUpdate" );
