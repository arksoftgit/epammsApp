...
if ( strLastWindow.equals("wSLC2UsageEntriesSelect") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSLC2UsageEntriesSelect strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSLC2UsageEntriesSelect LastWindow *** " + strLastWindow );
task.log().info("*** wSLC2UsageEntriesSelect LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSLC2UsageEntriesSelect", strActionToProcess );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "RemoveSLC_UsageEntries" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSLC2UsageEntriesSelect", strActionToProcess );
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSLC2UsageEntriesSelect.jsp", "wSLC2.RemoveSLC_UsageEntries" );
      try
      {
         nOptRC = wSLC2.RemoveSLC_UsageEntries( new zVIEW( vKZXMLPGO ) );
      }
      catch (Exception e)
      {
         // Set the error return code.
         nOptRC = 2;
         strVMLError = "<br><br>*** Error running Operation RemoveSLC_UsageEntries: " + e.getMessage();
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "SelectMLC_UsageEntries" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSLC2UsageEntriesSelect.jsp", "wSLC2.SelectMLC_UsageEntries" );
      try
      {
         nOptRC = wSLC2.SelectMLC_UsageEntries( new zVIEW( vKZXMLPGO ) );
      }
      catch (Exception e)
      {
         // Set the error return code.
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----------------------------------->>> " + "wSLC2UsageEntriesSelect" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSLC2UsageEntriesSelect.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSLC2UsageEntriesSelect.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<title>Usage Entries Select</title>
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSLC2UsageEntriesSelect.js"></script>
...
<%
   ...
   {
%>
       <li><a href="#"  onclick="AcceptAndReturn( )">Accept & Return</a></li>
<%
   }
%>
...
<%
   ...
   {
%>
       <li><a href="#"  onclick="CancelAndReturn( )">Cancel & Return</a></li>
<%
   }
%>
...
<form name="wSLC2UsageEntriesSelect" id="wSLC2UsageEntriesSelect" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSLC2", "UsageEntriesSelect" );
...
try
{
   ...
   if ( VmlOperation.isValid( mSubLC ) )
   {
      ...
      csrRC2 = vGrid1.cursor( "S_Usage" ).setFirst(  );
      while ( csrRC2.isSet() )
      {
...
<%
         csrRC2 = vGrid1.cursor( "S_Usage" ).setNextContinue( );
      }
      vGrid1.drop( );
   }
}
...
<div>  <!-- Beginning of a new line -->
<% /* PushBtn2:PushBtn */ %>
<button type="button" name="PushBtn2" id="PushBtn2" value="" onclick="RemoveSLC_UsageEntries( )" style="width:26px;height:32px;">-></button>
...
<%
   session.setAttribute( "ZeidonWindow", "wSLC2UsageEntriesSelect" );
