...
<%-- wMLCUsageGroupSelect   Generate Timestamp: 20160824153945379 --%>
         ...
         try
         {
            if ( webMapping )
               ...
               if ( strMapValue != null )
                  ...
                  vGridTmp.cursor( "M_UsageNonGroupUsage" ).getAttribute( "wSelected" ).setValue( "", "" );
         }
         catch ( InvalidAttributeValueException e )
         {
            nMapError = -16;
...
if ( strLastWindow.equals("wMLCUsageGroupSelect") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wMLCUsageGroupSelect strActionToProcess *** " + strActionToProcess );
task.log().info("*** wMLCUsageGroupSelect LastWindow *** " + strLastWindow );
task.log().info("*** wMLCUsageGroupSelect LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wMLCUsageGroupSelect" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wMLCUsageGroupSelect.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wMLCUsageGroupSelect.jsp") ) 
         {
            response.sendRedirect( strURL );
...
else
{
   ...
   if ( VmlOperation.isValid( wWebXA ) )
   {
      ...
      wWebXA.cursor( "Root" ).getAttribute( "CurrentWindow" ).setValue( "UsageGroupSelect", "" );
   }
...
<form name="wMLCUsageGroupSelect" id="wMLCUsageGroupSelect" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wMLC", "UsageGroupSelect" );
...
<%
   session.setAttribute( "ZeidonWindow", "wMLCUsageGroupSelect" );
