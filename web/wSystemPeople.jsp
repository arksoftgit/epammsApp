...
<%-- wSystemPeople   Generate Timestamp: 20160406142159028 --%>
...
if ( strLastWindow.equals("wSystemPeople") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSystemPeople strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSystemPeople LastWindow *** " + strLastWindow );
task.log().info("*** wSystemPeople LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "GoToPersons" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSystemPeople", strActionToProcess );
      ...
      // Next Window
      strNextJSP_Name = wSystem.SetWebRedirection( vKZXMLPGO, wSystem.zWAB_ReplaceWindowWithModalWindow, "wSystem", "Persons" );
      strURL = response.encodeRedirectURL( strNextJSP_Name );
      nRC = 1;  // do the redirection
      break;
   }

   while ( bDone == false && StringUtils.equals( strActionToProcess, "GoToUsers" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSystemPeople", strActionToProcess );
      ...
      // Next Window
      strNextJSP_Name = wSystem.SetWebRedirection( vKZXMLPGO, wSystem.zWAB_ReplaceWindowWithModalWindow, "wSystem", "Users" );
      strURL = response.encodeRedirectURL( strNextJSP_Name );
      nRC = 1;  // do the redirection
      break;
   }
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "smLogin" ) )
   {
      ...
      // Next Window
      strNextJSP_Name = wSystem.SetWebRedirection( vKZXMLPGO, wSystem.zWAB_StartTopWindow, "wStartUp", "UserLogin" );
      strURL = response.encodeRedirectURL( strNextJSP_Name );
      nRC = 1;  // do the redirection
      break;
   }
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wSystemPeople" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSystemPeople.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSystemPeople.jsp") ) 
         {
            response.sendRedirect( strURL );
...
else
{
   ...
   if ( VmlOperation.isValid( wWebXA ) )
   {
      ...
      wWebXA.cursor( "Root" ).getAttribute( "CurrentWindow" ).setValue( "People", "" );
   }
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSystemPeople.js"></script>
...
<form name="wSystemPeople" id="wSystemPeople" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSystem", "People" );
...
<span class="text14"  id="Text4" name="Text4" style="width:254px;height:16px;">System > People</span>
...
<div id="GroupBox4" name="GroupBox4" style="width:144px;height:88px;float:left;">  <!-- GroupBox4 --> 
...
<% /* PushBtn1:PushBtn */ %>
<button type="button" class="formStylebutton" name="PushBtn1" id="PushBtn1" value="" onclick="GoToPersons( )" style="width:114px;height:28px;">Persons</button>
...
<span style="height:28px;">&nbsp</span>
<% /* PushBtn4:PushBtn */ %>
<button type="button" class="formStylebutton" name="PushBtn4" id="PushBtn4" value="" onclick="GoToUsers( )" style="width:114px;height:28px;">Users</button>
...
<%
   session.setAttribute( "ZeidonWindow", "wSystemPeople" );
