...
if ( strLastWindow.equals("wMLC2IngredientsStatement") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wMLC2IngredientsStatement strActionToProcess *** " + strActionToProcess );
task.log().info("*** wMLC2IngredientsStatement LastWindow *** " + strLastWindow );
task.log().info("*** wMLC2IngredientsStatement LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wMLC2IngredientsStatement", strActionToProcess );
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wMLC2IngredientsStatement.jsp", "wMLC2.AcceptAddNewIngredient" );
      try
      {
         nOptRC = wMLC2.AcceptAddNewIngredient( new zVIEW( vKZXMLPGO ) );
      }
      catch (Exception e)
      {
         // Set the error return code.
         nOptRC = 2;
         strVMLError = "<br><br>*** Error running Operation AcceptAddNewIngredient: " + e.getMessage();
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----------------------------------->>> " + "wMLC2IngredientsStatement" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wMLC2IngredientsStatement.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wMLC2IngredientsStatement.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<script language="JavaScript" type="text/javascript" src="./genjs/wMLC2IngredientsStatement.js"></script>
...
<form name="wMLC2IngredientsStatement" id="wMLC2IngredientsStatement" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wMLC2", "IngredientsStatement" );
...
<div id="GBIngredientsStatement" name="GBIngredientsStatement"   style="float:left;position:relative; width:784px; height:162px;">  <!-- GBIngredientsStatement --> 
...
<label class="text14bold"  id="ActiveTitle" name="ActiveTitle" style="width:568px;height:20px;position:absolute;left:130px;top:28px;"><%=strTextDisplayValue%></label>
...
<label class="text14bold"  id="InertTitle" name="InertTitle" style="width:568px;height:20px;position:absolute;left:130px;top:48px;"><%=strTextDisplayValue%></label>
...
<textarea name="ChemicalName" id="ChemicalName"style="width:510px;height:32px;position:absolute;left:130px;top:70px;border:solid;border-width:2px;border-style:groove;" wrap="wrap"><%=strErrorMapValue%></textarea>

<% /* PBCleanHTML:PushBtn */ %>
<button type="button" name="PBCleanHTML" id="PBCleanHTML" value="" onclick="CleanIngredientsStmtHTML( )" style="width:102px;height:24px;position:absolute;left:650px;top:70px;">Clean HTML</button>

<% /* ActiveInert::Text */ %>

<label  id="ActiveInert:" name="ActiveInert:" style="width:118px;height:20px;position:absolute;left:8px;top:106px;">Active/Inert:</label>

<% /* IngredientsActive:ComboBox */ %>
<% strErrorMapValue = "";  %>

<select  name="IngredientsActive" id="IngredientsActive" size="1" style="width:108px;position:absolute;left:130px;top:106px;" onchange="IngredientsActiveOnChange( )">
...
<label  id="Precent:" name="Precent:" style="width:118px;height:20px;position:absolute;left:8px;top:128px;">Percent:</label>
...
<input class="text12" name="Percent" id="Percent" style="width:94px;position:absolute;left:130px;top:128px;<%=strErrorColor%>" type="text" value="<%=strErrorMapValue%>" >
...
<%
   session.setAttribute( "ZeidonWindow", "wMLC2IngredientsStatement" );
