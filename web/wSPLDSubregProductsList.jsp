...
if ( strLastWindow.equals("wSPLDSubregProductsList") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSPLDSubregProductsList strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSPLDSubregProductsList LastWindow *** " + strLastWindow );
task.log().info("*** wSPLDSubregProductsList LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSPLDSubregProductsList", strActionToProcess );
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLDSubregProductsList.jsp", "wSPLD.DeleteSubregProduct" );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "ReturnNoRefresh" ) )
   {
      ...
      // Next Window
      strNextJSP_Name = wSPLD.SetWebRedirection( vKZXMLPGO, wSPLD.zWAB_ReplaceWindowWithModalWindow, "wStartUp", "ePammsPortal" );
      strURL = response.encodeRedirectURL( strNextJSP_Name );
      nRC = 1;  // do the redirection
      break;
   }
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "GOTO_DeleteLLD" ) )
   {
      ...
      // Next Window
      strNextJSP_Name = wSPLD.SetWebRedirection( vKZXMLPGO, wSPLD.zWAB_StartModalSubwindow, "wSPLD", "DeleteLLD" );
      strURL = response.encodeRedirectURL( strNextJSP_Name );
      nRC = 1;  // do the redirection
      break;
   }
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "GOTO_UpdateSubregProduct" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLDSubregProductsList.jsp", "wSPLD.GOTO_UpdateSubregProduct" );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "NewSubregProduct" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLDSubregProductsList.jsp", "wSPLD.GOTO_NewSubregProduct" );
         nOptRC = wSPLD.GOTO_NewSubregProduct( new zVIEW( vKZXMLPGO ) );
      if ( nOptRC == 2 )
      {
         nRC = 2;  // do the "error" redirection
         session.setAttribute( "ZeidonError", "Y" );
      ...
      {
         // Next Window
         strNextJSP_Name = wSPLD.SetWebRedirection( vKZXMLPGO, wSPLD.zWAB_StartModalSubwindow, "wSubR", "NewSubregProduct" );
      }
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wSPLDSubregProductsList" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSPLDSubregProductsList.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSPLDSubregProductsList.jsp") ) 
         {
            response.sendRedirect( strURL );
...
{
   VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLDSubregProductsList.jsp", "wSPLD.InitListSubregProducts" );
   ...
   if ( nOptRC == 2 )
   {
      ...
      strURLParameters = "?CallingPage=wSPLDSubregProductsList.jsp" +
                         "&Message=" + strMessage +
                         "&DialogName=" + "wSPLD" +
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSPLDSubregProductsList.js"></script>
...
<%
   ...
   {
%>
       <li><a href="#"  onclick="ReturnNoRefresh( )">Return</a></li>
<%
   }
%>
...
<form name="wSPLDSubregProductsList" id="wSPLDSubregProductsList" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSPLD", "SubregProductsList" );
...
<%
   session.setAttribute( "ZeidonWindow", "wSPLDSubregProductsList" );
