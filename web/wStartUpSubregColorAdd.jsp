...
<%-- wStartUpSubregColorAdd   Generate Timestamp: 20160317151023587 --%>
...
if ( strLastWindow.equals("wStartUpSubregColorAdd") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wStartUpSubregColorAdd strActionToProcess *** " + strActionToProcess );
task.log().info("*** wStartUpSubregColorAdd LastWindow *** " + strLastWindow );
task.log().info("*** wStartUpSubregColorAdd LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wStartUpSubregColorAdd", strActionToProcess );
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wStartUpSubregColorAdd" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wStartUpSubregColorAdd.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wStartUpSubregColorAdd.jsp") ) 
         {
            response.sendRedirect( strURL );
...
else
{
   ...
   if ( nOptRC == 2 )
   {
      ...
      strURLParameters = "?CallingPage=wStartUpSubregColorAdd.jsp" +
                         "&Message=" + strMessage +
                         "&DialogName=" + "wStartUp" +
                         "&OperationName=" + "InitColorForAdd";
   ...
   if ( VmlOperation.isValid( wWebXA ) )
   {
      ...
      wWebXA.cursor( "Root" ).getAttribute( "CurrentWindow" ).setValue( "SubregColorAdd", "" );
   }
...
<title>Add Subregistrant Color</title>
...
<form name="wStartUpSubregColorAdd" id="wStartUpSubregColorAdd" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wStartUp", "SubregColorAdd" );
...
<div id="Color" name="Color"   style="float:left;position:relative; width:614px; height:232px;">  <!-- Color --> 
