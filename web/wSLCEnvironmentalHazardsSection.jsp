                           ...
                           boolean webMapping )
{
   ...
   if ( VmlOperation.isValid( mSubLC ) )
   {
      ...
      csrRC = vGridTmp.cursor( "S_GeneralSubStatement" ).setFirst(  );
      while ( csrRC.isSet() )
      {
         lEntityKey = vGridTmp.cursor( "S_GeneralSubStatement" ).getEntityKey( );
         strEntityKey = Long.toString( lEntityKey );
         iTableRowCnt++;

         csrRC = vGridTmp.cursor( "S_GeneralSubStatement" ).setNextContinue( );
      }
...
if ( strLastWindow.equals("wSLCEnvironmentalHazardsSection") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSLCEnvironmentalHazardsSection strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSLCEnvironmentalHazardsSection LastWindow *** " + strLastWindow );
task.log().info("*** wSLCEnvironmentalHazardsSection LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSLCEnvironmentalHazardsSection", strActionToProcess );
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wSLCEnvironmentalHazardsSection" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSLCEnvironmentalHazardsSection.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSLCEnvironmentalHazardsSection.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSLCEnvironmentalHazardsSection.js"></script>
...
<form name="wSLCEnvironmentalHazardsSection" id="wSLCEnvironmentalHazardsSection" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSLC", "EnvironmentalHazardsSection" );
...
try
{
   ...
   if ( VmlOperation.isValid( mSubLC ) )
   {
      ...
      vGridEnvironmental = mSubLC.newView( );
      csrRC2 = vGridEnvironmental.cursor( "S_GeneralSubStatement" ).setFirst(  );
      while ( csrRC2.isSet() )
      {
         ...
         if ( nRC >= 0 )
         {
            strGridEditEnvironmentalText = vGridEnvironmental.cursor( "S_GeneralSubStatement" ).getAttribute( "Text" ).getString( "" );
...
<%
         csrRC2 = vGridEnvironmental.cursor( "S_GeneralSubStatement" ).setNextContinue( );
      }
...
<%
   session.setAttribute( "ZeidonWindow", "wSLCEnvironmentalHazardsSection" );
