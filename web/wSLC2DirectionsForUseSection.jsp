...
if ( strLastWindow.equals("wSLC2DirectionsForUseSection") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSLC2DirectionsForUseSection strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSLC2DirectionsForUseSection LastWindow *** " + strLastWindow );
task.log().info("*** wSLC2DirectionsForUseSection LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSLC2DirectionsForUseSection", strActionToProcess );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "GOTO_DirectionsForUseStatement" ) )
   {
      ...
      // Next Window
      strNextJSP_Name = wSLC2.SetWebRedirection( vKZXMLPGO, wSLC2.zWAB_StartModalSubwindow, "wSLC2", "DirectionsForUseStatement" );
      strURL = response.encodeRedirectURL( strNextJSP_Name );
      nRC = 1;  // do the redirection
      break;
   }
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----------------------------------->>> " + "wSLC2DirectionsForUseSection" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSLC2DirectionsForUseSection.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSLC2DirectionsForUseSection.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSLC2DirectionsForUseSection.js"></script>
...
<%
   ...
   {
%>
       <li><a href="#"  onclick="CancelDirectionsUseSect( )">Return</a></li>
<%
   }
%>
...
<form name="wSLC2DirectionsForUseSection" id="wSLC2DirectionsForUseSection" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSLC2", "DirectionsForUseSection" );
...
<% strTextDisplayValue = "";
   ...
   else
   {
      ...
      try
      {
         strTextDisplayValue = mSubLC.cursor( "S_DirectionsForUseSection" ).getStringFromAttribute( "Name", "" );
      }
      catch (Exception e)
      {
...
try
{
   ...
   if ( VmlOperation.isValid( mSubLC ) )
   {
      ...
      while ( csrRC2.isSet() )
      {
         ...
         if ( nRC >= 0 )
         {
            strGridEditCtl1 = vGrid1.cursor( "S_ClaimsDrivingUsage" ).getStringFromAttribute( "ClaimsClassification", "" );
         ...
         if ( nRC >= 0 )
         {
            strGridEditCtl2 = vGrid1.cursor( "S_ClaimsDrivingUsage" ).getStringFromAttribute( "dDisplayUsageName", "" );
...
try
{
   ...
   if ( VmlOperation.isValid( mMasLC ) )
   {
      ...
      while ( csrRC2.isSet() )
      {
         ...
         if ( nRC >= 0 )
         {
            strGridEditCtl3 = vGrid2.cursor( "M_DrivingUsage" ).getStringFromAttribute( "ClaimsClassification", "" );
         ...
         if ( nRC >= 0 )
         {
            strGridEditCtl4 = vGrid2.cursor( "M_DrivingUsage" ).getStringFromAttribute( "dDisplayUsageName", "" );
...
<%
   session.setAttribute( "ZeidonWindow", "wSLC2DirectionsForUseSection" );
