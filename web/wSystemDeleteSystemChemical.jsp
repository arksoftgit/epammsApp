...
if ( strLastWindow.equals("wSystemDeleteSystemChemical") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSystemDeleteSystemChemical strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSystemDeleteSystemChemical LastWindow *** " + strLastWindow );
task.log().info("*** wSystemDeleteSystemChemical LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   if ( task != null )
   {
      ...
      if ( strFeedback != "" )
      {
         ...
         wSystem.SaveFeedback( "mOrganiz", "wSystem", "DeleteSystemChemical", strFeedback );
      }

   }

   while ( bDone == false && StringUtils.equals( strActionToProcess, "ConfirmDeleteChemical" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSystemDeleteSystemChemical", strActionToProcess );
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSystemDeleteSystemChemical.jsp", "wSystem.ConfirmDeleteSystemChemical" );
      try
      {
         nOptRC = wSystem.ConfirmDeleteSystemChemical( new zVIEW( vKZXMLPGO ) );
      }
      catch (Exception e)
      {
         // Set the error return code.
         nOptRC = 2;
         strVMLError = "<br><br>*** Error running Operation ConfirmDeleteSystemChemical: " + e.getMessage();
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "CancelDeleteChemical" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSystemDeleteSystemChemical", strActionToProcess );
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSystemDeleteSystemChemical.jsp", "wSystem.CancelDeleteSystemChemical" );
      try
      {
         nOptRC = wSystem.CancelDeleteSystemChemical( new zVIEW( vKZXMLPGO ) );
      }
      catch (Exception e)
      {
         // Set the error return code.
         nOptRC = 2;
         strVMLError = "<br><br>*** Error running Operation CancelDeleteSystemChemical: " + e.getMessage();
         task.log().info( strVMLError );
      }
      if ( nOptRC == 2 )
      {
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "smConfirmDeleteChemical" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSystemDeleteSystemChemical", strActionToProcess );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "smCancelDeleteChemical" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSystemDeleteSystemChemical", strActionToProcess );
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----------------------------------->>> " + "wSystemDeleteSystemChemical" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSystemDeleteSystemChemical.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSystemDeleteSystemChemical.jsp") ) 
         {
            response.sendRedirect( strURL );
...
{
   VmlOperation.SetZeidonSessionAttribute( null, task, "wSystemDeleteSystemChemical.jsp", "wSystem.InitSystemChemicalForDelete" );
   nOptRC = wSystem.InitSystemChemicalForDelete( new zVIEW( vKZXMLPGO ) );
   if ( nOptRC == 2 )
   {
      View vView;
      String strMessage;
      ...
      strURLParameters = "?CallingPage=wSystemDeleteSystemChemical.jsp" +
                         "&Message=" + strMessage +
                         "&DialogName=" + "wSystem" +
                         "&OperationName=" + "InitSystemChemicalForDelete";
   ...
   if ( VmlOperation.isValid( wWebXA ) )
   {
      ...
      wWebXA.cursor( "Root" ).setAttribute( "CurrentWindow", "DeleteSystemChemical" );
   }
...
<title>Delete System Chemical</title>
...
<%
   csrRC = vKZXMLPGO.cursor( "DisableMenuOption" ).setFirst( "MenuOptionName", "CancelDeleteChemical" );
   if ( !csrRC.isSet() ) //if ( nRC < 0 )
   {
%>
...
<form name="wSystemDeleteSystemChemical" id="wSystemDeleteSystemChemical" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSystem", "DeleteSystemChemical" );
...
<div id="Chemical" name="Chemical" style="width:344px;height:110px;float:left;">  <!-- Chemical --> 
...
<span  id="Family:" name="Family:" style="width:130px;height:16px;">Family:</span>

<span style="height:16px;">&nbsp&nbsp&nbsp&nbsp&nbsp</span>
<% /* Family:EditBox */ %>
<%
   strErrorMapValue = VmlOperation.CheckError( "Family", strError );
   if ( !StringUtils.isBlank( strErrorMapValue ) )
   {
      if ( StringUtils.equals( strErrorFlag, "Y" ) )
         strErrorColor = "color:red;";
   ...
   else
   {
      ...
      else
      {
         ...
         if ( nRC >= 0 )
         {
            try
            {
            strErrorMapValue = mEPA.cursor( "EPA_ChemicalFamily" ).getStringFromAttribute( "ChemicalFamily", "" );
            }
            catch (Exception e)
            {
