...
if ( strLastWindow.equals("wSPLD2SPLD_Components") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSPLD2SPLD_Components strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSPLD2SPLD_Components LastWindow *** " + strLastWindow );
task.log().info("*** wSPLD2SPLD_Components LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSPLD2SPLD_Components", strActionToProcess );

      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLD2SPLD_Components.jsp", "wSPLD2.CancelSPLD" );
      try
      {
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "GOTO_UpdateSPLD_Statement" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLD2SPLD_Components.jsp", "wSPLD2.GOTO_UpdateSPLD_Statement" );
      try
      {
         nOptRC = wSPLD2.GOTO_UpdateSPLD_Statement( new zVIEW( vKZXMLPGO ) );
      }
      catch (Exception e)
      {
         // Set the error return code.
         nOptRC = 2;
         strVMLError = "<br><br>*** Error running Operation GOTO_UpdateSPLD_Statement: " + e.getMessage();
         task.log().info( strVMLError );
      }
      if ( nOptRC == 2 )
      {
      ...
      {
         // Next Window
         strNextJSP_Name = wSPLD2.SetWebRedirection( vKZXMLPGO, wSPLD2.zWAB_StartModalSubwindow, "wSPLD2", "SPLD_StatementUpdate" );
      }
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "REFRESH_SPLD_FromSLC" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLD2SPLD_Components.jsp", "wSPLD2.REFRESH_SPLD_FromSLC" );
      try
      {
         nOptRC = wSPLD2.REFRESH_SPLD_FromSLC( new zVIEW( vKZXMLPGO ) );
      }
      catch (Exception e)
      {
         // Set the error return code.
         nOptRC = 2;
         strVMLError = "<br><br>*** Error running Operation REFRESH_SPLD_FromSLC: " + e.getMessage();
         task.log().info( strVMLError );
      }
      if ( nOptRC == 2 )
      {
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "DisplayLLD" ) )
   {
      ...
      // Next Window
      strNextJSP_Name = wSPLD2.SetWebRedirection( vKZXMLPGO, wSPLD2.zWAB_ReplaceWindowWithModalWindow, "wSPLD2", "SPLD_UpdateLLD" );
      strURL = response.encodeRedirectURL( strNextJSP_Name );
      nRC = 1;  // do the redirection
      break;
   }
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "SaveAndReturn" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLD2SPLD_Components.jsp", "wSPLD2.SaveSPLD" );
      try
      {
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----------------------------------->>> " + "wSPLD2SPLD_Components" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSPLD2SPLD_Components.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSPLD2SPLD_Components.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSPLD2SPLD_Components.js"></script>
...
<form name="wSPLD2SPLD_Components" id="wSPLD2SPLD_Components" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSPLD2", "SPLD_Components" );
...
<%
   session.setAttribute( "ZeidonWindow", "wSPLD2SPLD_Components" );
