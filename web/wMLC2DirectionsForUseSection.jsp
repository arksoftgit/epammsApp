...
if ( strLastWindow.equals("wMLC2DirectionsForUseSection") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wMLC2DirectionsForUseSection strActionToProcess *** " + strActionToProcess );
task.log().info("*** wMLC2DirectionsForUseSection LastWindow *** " + strLastWindow );
task.log().info("*** wMLC2DirectionsForUseSection LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wMLC2DirectionsForUseSection", strActionToProcess );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "GOTO_DirsForUseStatementAdd" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wMLC2DirectionsForUseSection.jsp", "wMLC2.GOTO_DirsForUseStatementAdd" );
      try
      {
         nOptRC = wMLC2.GOTO_DirsForUseStatementAdd( new zVIEW( vKZXMLPGO ) );
      }
      catch (Exception e)
      {
      ...
      {
         // Next Window
         strNextJSP_Name = wMLC2.SetWebRedirection( vKZXMLPGO, wMLC2.zWAB_StartModalSubwindow, "wMLC2", "DirectionsForUseStatement" );
      }
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "GOTO_SelectRemoveDrivingDU" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wMLC2DirectionsForUseSection.jsp", "wMLC2.GOTO_SelectRemoveDrivingDU" );
      try
      {
         nOptRC = wMLC2.GOTO_SelectRemoveDrivingDU( new zVIEW( vKZXMLPGO ) );
      }
      catch (Exception e)
      {
         // Set the error return code.
         nOptRC = 2;
         strVMLError = "<br><br>*** Error running Operation GOTO_SelectRemoveDrivingDU: " + e.getMessage();
         task.log().info( strVMLError );
      }
      if ( nOptRC == 2 )
      {
      ...
      {
         // Next Window
         strNextJSP_Name = wMLC2.SetWebRedirection( vKZXMLPGO, wMLC2.zWAB_StartModalSubwindow, "wMLC2", "DirsForUseDrivingUsageSelect" );
      }
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----------------------------------->>> " + "wMLC2DirectionsForUseSection" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wMLC2DirectionsForUseSection.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wMLC2DirectionsForUseSection.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<script language="JavaScript" type="text/javascript" src="./genjs/wMLC2DirectionsForUseSection.js"></script>
...
<form name="wMLC2DirectionsForUseSection" id="wMLC2DirectionsForUseSection" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wMLC2", "DirectionsForUseSection" );
...
try
{
   ...
   if ( VmlOperation.isValid( mMasLC ) )
   {
      ...
      while ( csrRC2.isSet() )
      {
         ...
         if ( nRC >= 0 )
         {
            strGridEditCtl1 = vGrid1.cursor( "M_DrivingUsage" ).getStringFromAttribute( "ClaimsClassification", "" );
         ...
         if ( nRC >= 0 )
         {
            strGridEditCtl2 = vGrid1.cursor( "M_DrivingUsage" ).getStringFromAttribute( "Name", "" );
...
<%
   session.setAttribute( "ZeidonWindow", "wMLC2DirectionsForUseSection" );
