                           ...
                           boolean webMapping )
{
   ...
   if ( VmlOperation.isValid( mSPLDef ) )
   {
      ...
      if ( nRC >= 0 ) // CursorResult.SET
      {
         strMapValue = request.getParameter( "MLEdit4" );
         try
         {
            if ( webMapping )
         ...
         {
            nMapError = -16;
            VmlOperation.CreateMessage( task, "MLEdit4", e.getReason( ), strMapValue );
         }
      }
...
if ( strLastWindow.equals("wSPLD2SPLD_StatementUpdate") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSPLD2SPLD_StatementUpdate strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSPLD2SPLD_StatementUpdate LastWindow *** " + strLastWindow );
task.log().info("*** wSPLD2SPLD_StatementUpdate LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "ACCEPT_SPLD_StatementUpdate" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSPLD2SPLD_StatementUpdate", strActionToProcess );
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLD2SPLD_StatementUpdate.jsp", "wSPLD2.ACCEPT_SPLD_StatementUpdate" );
      try
      {
         nOptRC = wSPLD2.ACCEPT_SPLD_StatementUpdate( new zVIEW( vKZXMLPGO ) );
      }
      catch (Exception e)
      {
         // Set the error return code.
         nOptRC = 2;
         strVMLError = "<br><br>*** Error running Operation ACCEPT_SPLD_StatementUpdate: " + e.getMessage();
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "SPLIT_StatementForContinuation" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSPLD2SPLD_StatementUpdate", strActionToProcess );
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSPLD2SPLD_StatementUpdate.jsp", "wSPLD2.SPLIT_StatementForContinuation" );
      try
      {
         nOptRC = wSPLD2.SPLIT_StatementForContinuation( new zVIEW( vKZXMLPGO ) );
      }
      catch (Exception e)
      {
         // Set the error return code.
         nOptRC = 2;
         strVMLError = "<br><br>*** Error running Operation SPLIT_StatementForContinuation: " + e.getMessage();
         task.log().info( strVMLError );
      }
      if ( nOptRC == 2 )
      {
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----------------------------------->>> " + "wSPLD2SPLD_StatementUpdate" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSPLD2SPLD_StatementUpdate.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSPLD2SPLD_StatementUpdate.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSPLD2SPLD_StatementUpdate.js"></script>
...
<%
   ...
   {
%>
       <li><a href="#"  onclick="Cancel( )">Cancel and Retrurn</a></li>
<%
   }
%>
...
<form name="wSPLD2SPLD_StatementUpdate" id="wSPLD2SPLD_StatementUpdate" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSPLD2", "SPLD_StatementUpdate" );
...
<div id="Registrants1" name="Registrants1" style="width:610px;height:92px;float:left;">  <!-- Registrants1 --> 
...
<span  id="PrimaryRegistrantName:3" name="PrimaryRegistrantName:3" style="width:130px;height:16px;"></span>

</td>
<td valign="top"  class="text14bold" style="width:366px;">
<% /* TXName3:Text */ %>

<span class="text14bold"  id="TXName3" name="TXName3" style="width:366px;height:16px;">Statement Detail</span>
...
<span  id="Subregistrant:3" name="Subregistrant:3" style="width:130px;height:16px;">Title:</span>
...
<span  id="Text5" name="Text5" style="width:130px;height:18px;">Continuation:</span>
...
<select  name="ComboBox1" id="ComboBox1" size="1" style="width:370px;" onchange="ComboBox1OnChange( )">

<%
   mSPLDef = task.getViewByName( "mSPLDef" );
   if ( VmlOperation.isValid( mSPLDef ) )
   {
      List<TableEntry> list = JspWebUtils.getTableDomainValues( mSPLDef , "CompositeComponentList", "ContinuationBreakFlag", "" );
...
<div>  <!-- Beginning of a new line -->
<% /* GroupBox2:GroupBox */ %>
<div id="GroupBox2" name="GroupBox2" style="float:left;width:610px;" >

<table cols=1 style="width:610px;"  class="grouptable">

<tr>
<td valign="top" style="width:176px;">
<% /* TypePrompt1:Text */ %>

<span  id="TypePrompt1" name="TypePrompt1" style="width:176px;height:20px;">Full Statement Text</span>
...
<div id="GroupBox4" name="GroupBox4" style="width:274px;height:40px;float:left;">  <!-- GroupBox4 --> 
...
<% /* PushBtn1:PushBtn */ %>
<button type="button" name="PushBtn1" id="PushBtn1" value="" onclick="SPLIT_StatementForContinuation( )" style="width:262px;height:20px;">Split Above Text at Break Characters</button>
...
<div id="GroupBox5" name="GroupBox5" style="width:314px;height:40px;float:left;">  <!-- GroupBox5 --> 
...
<span  id="Text7" name="Text7" style="width:290px;height:20px;">( Break characters are two vertical lines: || )</span>
...
<span  id="Text1" name="Text1" style="width:246px;height:20px;">Continuation Leading Text</span>
...
<div>  <!-- Beginning of a new line -->
<% /* GroupBox3:GroupBox */ %>
<div id="GroupBox3" name="GroupBox3" style="float:left;width:610px;" >
...
<span  id="Text3" name="Text3" style="width:246px;height:20px;">Continuation Following Text</span>
...
<%
   session.setAttribute( "ZeidonWindow", "wSPLD2SPLD_StatementUpdate" );
