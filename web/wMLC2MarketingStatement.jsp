...
if ( strLastWindow.equals("wMLC2MarketingStatement") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wMLC2MarketingStatement strActionToProcess *** " + strActionToProcess );
task.log().info("*** wMLC2MarketingStatement LastWindow *** " + strLastWindow );
task.log().info("*** wMLC2MarketingStatement LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wMLC2MarketingStatement", strActionToProcess );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "GOTO_SelectRemoveMktgEntries" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wMLC2MarketingStatement.jsp", "wMLC2.GOTO_SelectRemoveMktgEntries" );
      try
      {
         nOptRC = wMLC2.GOTO_SelectRemoveMktgEntries( new zVIEW( vKZXMLPGO ) );
      }
      catch (Exception e)
      {
      ...
      {
         // Next Window
         strNextJSP_Name = wMLC2.SetWebRedirection( vKZXMLPGO, wMLC2.zWAB_StartModalSubwindow, "wMLC2", "MarketingSelectByType" );
      }
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----------------------------------->>> " + "wMLC2MarketingStatement" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wMLC2MarketingStatement.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wMLC2MarketingStatement.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<script language="JavaScript" type="text/javascript" src="./genjs/wMLC2MarketingStatement.js"></script>
...
<form name="wMLC2MarketingStatement" id="wMLC2MarketingStatement" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wMLC2", "MarketingStatement" );
...
try
{
   ...
   if ( VmlOperation.isValid( mMasLC ) )
   {
      ...
      while ( csrRC2.isSet() )
      {
         ...
         if ( nRC >= 0 )
         {
            strGridEditCtl4 = vGrid3.cursor( "M_MarketingUsage" ).getStringFromAttribute( "dDisplayUsageName", "" );
...
<%
   session.setAttribute( "ZeidonWindow", "wMLC2MarketingStatement" );
