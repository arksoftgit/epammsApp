...
if ( strLastWindow.equals("wMLC2MarketingSection") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wMLC2MarketingSection strActionToProcess *** " + strActionToProcess );
task.log().info("*** wMLC2MarketingSection LastWindow *** " + strLastWindow );
task.log().info("*** wMLC2MarketingSection LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wMLC2MarketingSection", strActionToProcess );
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "GOTO_MarketingStatementAdd" ) )
   {
      ...
      // Next Window
      strNextJSP_Name = wMLC2.SetWebRedirection( vKZXMLPGO, wMLC2.zWAB_StartModalSubwindow, "wMLC2", "MarketingStatement" );
      strURL = response.encodeRedirectURL( strNextJSP_Name );
      nRC = 1;  // do the redirection
      break;
   }

   while ( bDone == false && StringUtils.equals( strActionToProcess, "GOTO_MarketingStatementDelete" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wMLC2MarketingSection", strActionToProcess );
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wMLC2MarketingSection.jsp", "wMLC2.GOTO_MarketingStatementDelete" );
      try
      {
         nOptRC = wMLC2.GOTO_MarketingStatementDelete( new zVIEW( vKZXMLPGO ) );
      }
      catch (Exception e)
      {
         // Set the error return code.
         nOptRC = 2;
         strVMLError = "<br><br>*** Error running Operation GOTO_MarketingStatementDelete: " + e.getMessage();
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----------------------------------->>> " + "wMLC2MarketingSection" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wMLC2MarketingSection.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wMLC2MarketingSection.jsp") ) 
         {
            response.sendRedirect( strURL );
...
<script language="JavaScript" type="text/javascript" src="./genjs/wMLC2MarketingSection.js"></script>
...
<form name="wMLC2MarketingSection" id="wMLC2MarketingSection" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wMLC2", "MarketingSection" );
...
<tr<%=strOdd%>>
   ...
   <td nowrap><a href="#" style="display:block;width:100%;height:100%;text-decoration:none;" name="BMBDeleteDirectionsUseStatement" onclick="GOTO_MarketingStatementDelete( this.id )" id="BMBDeleteDirectionsUseStatement::<%=strEntityKey%>"><img src="./images/ePammsDelete.jpg" alt="Delete"></a></td>
...
<%
   session.setAttribute( "ZeidonWindow", "wMLC2MarketingSection" );
