...
if ( strLastWindow.equals("wSLCUpdateSubregLabelContent") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSLCUpdateSubregLabelContent strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSLCUpdateSubregLabelContent LastWindow *** " + strLastWindow );
task.log().info("*** wSLCUpdateSubregLabelContent LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSLCUpdateSubregLabelContent", strActionToProcess );

      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSLCUpdateSubregLabelContent.jsp", "wSLC.CancelNewSubregLabelContent" );
      try
      {
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "GenerateNewSLC_FromMLC" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSLCUpdateSubregLabelContent.jsp", "wSLC.GenerateNewSLC_FromMLC" );
      try
      {
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "AcceptNewSubregLabelContent" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSLCUpdateSubregLabelContent", strActionToProcess );
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSLCUpdateSubregLabelContent.jsp", "wSLC.AcceptNewSubregLabelContent" );
      try
      {
         nOptRC = wSLC.AcceptNewSubregLabelContent( new zVIEW( vKZXMLPGO ) );
      }
      catch (Exception e)
      {
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "smAcceptNewSubregLabelContent" ) )
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSLCUpdateSubregLabelContent", strActionToProcess );
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----------------------------------->>> " + "wSLCUpdateSubregLabelContent" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSLCUpdateSubregLabelContent.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSLCUpdateSubregLabelContent.jsp") ) 
         {
            response.sendRedirect( strURL );
...
{
   VmlOperation.SetZeidonSessionAttribute( null, task, "wSLCUpdateSubregLabelContent.jsp", "wSLC.InitSubregLabelContentForUpdate" );
   ...
   if ( nOptRC == 2 )
   {
      ...
      strURLParameters = "?CallingPage=wSLCUpdateSubregLabelContent.jsp" +
                         "&Message=" + strMessage +
                         "&DialogName=" + "wSLC" +
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSLCUpdateSubregLabelContent.js"></script>
...
<%
   ...
   {
%>
       <li><a href="#"  onclick="smAcceptNewSubregLabelContent( )">Save and Return</a></li>
<%
   }
%>
...
<form name="wSLCUpdateSubregLabelContent" id="wSLCUpdateSubregLabelContent" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSLC", "UpdateSubregLabelContent" );
...
<div id="SubregLabelContent" name="SubregLabelContent" class="withborder" style="width:730px;height:110px;float:left;">  <!-- SubregLabelContent --> 

<div  id="SubregLabelContent" name="SubregLabelContent" >Subregistrant Product Label Content</div>
...
<%
   session.setAttribute( "ZeidonWindow", "wSLCUpdateSubregLabelContent" );
