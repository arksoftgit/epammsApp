...
<%-- wSLCMarketingKeywordSelect   Generate Timestamp: 20160620105928500 --%>
...
if ( strLastWindow.equals("wSLCMarketingKeywordSelect") && StringUtils.isBlank( strActionToProcess ) && StringUtils.isBlank( strLastAction ) )
{
...
task.log().info("*** wSLCMarketingKeywordSelect strActionToProcess *** " + strActionToProcess );
task.log().info("*** wSLCMarketingKeywordSelect LastWindow *** " + strLastWindow );
task.log().info("*** wSLCMarketingKeywordSelect LastAction *** " + strLastAction );
...
if ( strActionToProcess != null )
{
   ...
   {
      bDone = true;
      VmlOperation.SetZeidonSessionAttribute( session, task, "wSLCMarketingKeywordSelect", strActionToProcess );
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSLCMarketingKeywordSelect", "wSLC.RemoveMarketingKeywordEntries" );
      nOptRC = wSLC.RemoveMarketingKeywordEntries( new zVIEW( vKZXMLPGO ) );
      if ( nOptRC == 2 )
      {
   ...
   while ( bDone == false && StringUtils.equals( strActionToProcess, "SelectMarketingUsageEntries" ) )
   {
      ...
      // Action Operation
      nRC = 0;
      VmlOperation.SetZeidonSessionAttribute( null, task, "wSLCMarketingKeywordSelect", "wSLC.SelectMarketingKeywordEntries" );
      nOptRC = wSLC.SelectMarketingKeywordEntries( new zVIEW( vKZXMLPGO ) );
      if ( nOptRC == 2 )
      {
         nRC = 2;  // do the "error" redirection
         session.setAttribute( "ZeidonError", "Y" );
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnUnload" ) )
   {
      ...
      if ( task != null )
      {
         task.log().info( "OnUnload UnregisterZeidonApplication: ----->>> " + "wSLCMarketingKeywordSelect" );
         task.dropTask();
         task = null;
   ...
   while ( bDone == false && strActionToProcess.equals( "_OnResubmitPage" ) )
   {
      ...
      strURL = response.encodeRedirectURL( "wSLCMarketingKeywordSelect.jsp" );
      nRC = 1;  //do the redirection
      break;
   }
   ...
   if ( nRC != 0 )
   {
      if ( nRC > 0 )
      {
         ...
         if ( ! strURL.equals("wSLCMarketingKeywordSelect.jsp") ) 
         {
            response.sendRedirect( strURL );
...
else
{
   ...
   if ( VmlOperation.isValid( wWebXA ) )
   {
      ...
      wWebXA.cursor( "Root" ).getAttribute( "CurrentWindow" ).setValue( "MarketingKeywordSelect", "" );
   }
...
<title>MarketingKeywordSelect</title>
...
<script language="JavaScript" type="text/javascript" src="./genjs/wSLCMarketingKeywordSelect.js"></script>
...
<form name="wSLCMarketingKeywordSelect" id="wSLCMarketingKeywordSelect" method="post">
...
<%
   ...
   strFocusCtrl = VmlOperation.GetFocusCtrl( task, "wSLC", "MarketingKeywordSelect" );
...
<span  id="Text1" name="Text1" style="width:382px;height:22px;">Currently Selected SLC  Keyword Entries</span>
...
<thead><tr>
   ...
   <th style="width:142px;">Keyword</th>
   <th style="width:206px;">Text</th>
...
<span  id="Text2" name="Text2" style="width:306px;height:24px;">Potential SLC Keywords from MLC</span>
...
<%
   session.setAttribute( "ZeidonWindow", "wSLCMarketingKeywordSelect" );
